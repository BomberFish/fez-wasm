...
namespace FezGame.Components.Actions
{
	internal class DropTrile : PlayerAction
	{
		...
		protected override void TestConditions()
		{
			...
			if (((uint)(actionType - 12) > 3u && (uint)(actionType - 17) > 3u) || base.PlayerManager.Background || base.InputManager.GrabThrow != FezButtonState.Pressed || (!(base.InputManager.Movement.Y < -0.5f) && !((double)Math.Abs(base.InputManager.Movement.X) < 0.25)))
			{
				return;
			}
