	...
	public class EnterDoor : PlayerAction
	{
		...
		protected override void TestConditions()
		{
			...
			case ActionType.IdleYawn:
			{
				...
				if (base.PlayerManager.NextLevel == "SKULL_B")
				{
					ITombstoneService tombstoneService = ServiceHelper.Get<ITombstoneService>();
					if (tombstoneService.get_AlignedCount() < 4)
					{
						UnDotize();
						break;
					}
		...
		{
			IWaiter waiter = Waiters.Interpolate(1.75, delegate(float s)
			{
		...
		{
			return type.IsEnteringDoor() || step != -1f;
		}
		...
		private void DoLoad(bool dummy)
		{
			...
			if (surface != null && surface.Trile.ActorSettings.Type == ActorType.UnlockedDoor)
			{
				FaceOrientation faceOrientation = FezMath.OrientationFromPhi(surface.Trile.ActorSettings.Face.ToPhi() + surface.Phi);
				if (faceOrientation == base.CameraManager.Viewpoint.VisibleOrientation())
				{
