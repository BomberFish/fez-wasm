	...
	public class Grab : PlayerAction
	{
		...
		protected override void TestConditions()
		{
			...
			case ActionType.IdleYawn:
			{
				...
				{
					FaceOrientation faceOrientation = FezMath.OrientationFromPhi(destination.Trile.ActorSettings.Face.ToPhi() + destination.Phi);
					FaceOrientation faceOrientation2 = base.CameraManager.Viewpoint.GetRotatedView((base.PlayerManager.LookingDirection != HorizontalDirection.Right) ? 1 : (-1)).VisibleOrientation();
					if (faceOrientation != faceOrientation2)
					{
						break;
					}
				}
