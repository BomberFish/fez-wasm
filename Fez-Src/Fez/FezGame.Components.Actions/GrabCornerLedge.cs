...
namespace FezGame.Components.Actions
{
	public class GrabCornerLedge : PlayerAction
	{
		...
		protected override void TestConditions()
		{
			...
			if (actionType != ActionType.Jumping && actionType != ActionType.Falling)
			{
				return;
			}
			...
			Vector3 vector3 = vector2 * lookingDirection.Sign();
			float num = base.PlayerManager.Velocity.Dot(vector3);
			...
			TrileInstance trileInstance3 = base.LevelManager.ActualInstanceAt(trileInstance.Center - vector3);
			TrileInstance deep = base.LevelManager.NearestTrile(trileInstance.Center - vector3).Deep;
			if ((deep != null && deep.Enabled && deep.GetRotatedFace(base.CameraManager.VisibleOrientation) != CollisionType.None) || (trileInstance3 != null && trileInstance3.Enabled && !trileInstance3.Trile.Immaterial))
			{
				return;
			}
			if (base.PlayerManager.Action == ActionType.Jumping)
			{
				float num3 = ((trileInstance.Center - base.PlayerManager.LeaveGroundPosition) * vector).Length();
				if ((double)num3 < 1.25)
				{
					return;
				}
			}
			if (trileInstance.GetRotatedFace(visibleOrientation) != 0)
			{
				MultipleHits<CollisionResult> result = base.CollisionManager.CollideEdge(trileInstance.Center + trileInstance.TransformedSize * (Vector3.UnitY * 0.498f + vector3 * 0.5f), Vector3.Down * Math.Sign(base.CollisionManager.GravityFactor), base.PlayerManager.Size * FezMath.XZMask / 2f, Direction2D.Vertical);
				if (result.AnyHit())
				{
					return;
				}
			}
			Vector3 vector4 = (-vector3 + Vector3.UnitY * Math.Sign(base.CollisionManager.GravityFactor)) * trileInstance.TransformedSize / 2f;
			Vector3 vector5 = base.PlayerManager.Center * vector;
			Vector3 vector6 = trileInstance.Center * vector;
			float num4 = (vector5 - (vector6 + vector4)).Length();
			if (!(num4 < 0.35f))
			{
				return;
			}
		...
		protected override bool Act(TimeSpan elapsed)
		{
			...
			if (base.CameraManager.ActionRunning && !flag && ((base.PlayerManager.LookingDirection == HorizontalDirection.Right && base.InputManager.Right == fezButtonState) || (base.PlayerManager.LookingDirection == HorizontalDirection.Left && base.InputManager.Left == fezButtonState)))
			{
				...
				if (!background2)
				{
					...
					if ((deep == null || deep.GetRotatedFace(face) != 0) && deep2 != null && deep2.GetRotatedFace(face) == CollisionType.TopOnly)
					{
						MultipleHits<CollisionResult> result = base.CollisionManager.CollideEdge(deep2.Center, Vector3.Down * Math.Sign(base.CollisionManager.GravityFactor), base.PlayerManager.Size * FezMath.XZMask / 2f, Direction2D.Vertical);
						if (!result.AnyHit())
						{
