...
namespace FezGame.Components.Actions
{
	public class GrabStraightLedge : PlayerAction
	{
		...
		protected override void TestConditions()
		{
			...
			if ((actionType != ActionType.Jumping && actionType != ActionType.Falling) || !base.InputManager.Up.IsDown())
			{
				return;
			}
			...
			if ((deep != null && deep.GetRotatedFace(face) == CollisionType.AllSides) || deep2 == null || deep2.GetRotatedFace(face) != CollisionType.TopOnly)
			{
				return;
			}
			...
			{
				float num = ((deep2.Center - base.PlayerManager.LeaveGroundPosition) * base.CameraManager.Viewpoint.ScreenSpaceMask()).Length();
				if ((double)num < 1.25)
				{
					return;
				}
