	...
	internal class LesserWarp : PlayerAction
	{
		...
		private void TryInitialize()
		{
			...
			if (GateAo != null)
			{
				...
				MaskMesh = new Mesh
				{
					...
					Rotation = Quaternion.CreateFromAxisAngle(Vector3.Right, -MathF.PI / 2f)
				};
		...
		protected override bool Act(TimeSpan elapsed)
		{
			...
			case Phases.Rise:
			{
				...
				GateAo.Rotation = Quaternion.CreateFromAxisAngle(RiseAxis, RiseStep * (MathF.PI / 2f));
				MaskMesh.Position = GateAo.Position - Vector3.UnitY * 1.25f * (float)Math.Cos(RiseStep * (MathF.PI / 2f));
				...
				Vector3 vector = Vector3.Transform(new Vector3((float)Math.Cos(GateAngle), 0f, (float)Math.Sin(MathF.PI + GateAngle)), Quaternion.CreateFromAxisAngle(Vector3.Up, RisePhi - MathF.PI / 2f));
				GateAo.Position += vector * 1.25f * (float)Math.Sin(RiseStep * (MathF.PI / 2f));
			...
			case Phases.Warping:
			{
				...
				if (Phase != Phases.Warping && SinceStarted.TotalSeconds > 4.0)
				{
					...
					screenFade.Faded = (Action)Delegate.Combine(screenFade.Faded, (Action)delegate
					{
			...
			case Phases.Decelerate:
			{
				...
				MaskMesh.Position = GateAo.Position - Vector3.UnitY * 1.25f * (float)Math.Cos(RiseStep * (MathF.PI / 2f));
				MaskMesh.Rotation = GateAo.Rotation;
				float num4 = ((GateAngle < MathF.PI) ? 0f : (MathF.PI * 2f));
		...
		{
			return type == ActionType.LesserWarp || Phase != Phases.None;
		}
	}
}
