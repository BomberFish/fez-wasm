...
namespace FezGame.Components.Actions
{
	internal class OpenDoor : PlayerAction
	{
		...
		{
			sinceOpened += elapsed;
			float num = sinceOpened.Ticks;
			TimeSpan openingDuration = OpeningDuration;
			float num2 = FezMath.Saturate(num / (float)openingDuration.Ticks);
			if (doorBottom.InstanceId != -1)
			{
				float num3 = Easing.EaseInOut(num2, EasingType.Sine);
				float num4 = num3 * (MathF.PI / 2f);
				float num7 = (doorBottom.Phi = (doorTop.Phi = initialPhi + num4));
				if (aoInstance != null)
				{
					aoInstance.Rotation = Quaternion.CreateFromAxisAngle(Vector3.Up, num4) * aoInitialRotation;
				}
				...
				Vector3 vector4 = vector2 * 1.125f * num3 + vector * (float)(Math.Sin(num4 * 2f) * ((Math.Sqrt(2.0) - 1.0) / 2.0) - (double)(0.1875f * num3));
				...
				if (aoInstance != null)
				{
					vector4 = vector2 * 1.125f * num3 + vector * (float)(Math.Sin(num4 * 2f) * ((Math.Sqrt(2.0) - 1.0) / 2.0) + (double)(0.1875f * num3));
