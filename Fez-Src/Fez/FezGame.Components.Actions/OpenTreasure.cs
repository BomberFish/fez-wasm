	...
	internal class OpenTreasure : PlayerAction
	{
			...
			: base(game)
		{
			...
			for (int i = 0; i < 8; i++)
			{
				...
				float num2 = ((float)i + 0.5f) * (MathF.PI * 2f) / 8f;
			...
			group.Rotation = Quaternion.CreateFromAxisAngle(Vector3.Up, -MathF.PI / 2f);
			...
			base.LevelManager.LevelChanged += delegate
			{
				foreach (int inactiveArtObject in base.GameState.SaveData.ThisLevel.InactiveArtObjects)
				{
					if (inactiveArtObject >= 0 && base.LevelManager.ArtObjects.TryGetValue(inactiveArtObject, out var value) && value.ArtObject.ActorType == ActorType.TreasureChest)
					{
						...
						value.Rotation *= Quaternion.CreateFromAxisAngle(Vector3.UnitX, MathF.PI * -3f / 4f);
				...
				foreach (ArtObjectInstance artObjectInstance in array)
				{
					...
					else
					{
						...
						i.Position = artObjectInstance.Position;
						string mapName = artObjectInstance.ActorSettings.TreasureMapName;
						DrawActionScheduler.Schedule(delegate
						{
							i.Effect = new DefaultEffect.LitTextured();
							...
							for (int k = 0; k < i.Groups.Count; k++)
							{
								i.Groups[k].Texture = ((k % 2 == 0) ? texture2D : texture2D2);
							}
						});
						i.CustomData = new TreasureActorSettings
						{
		...
		protected override void TestConditions()
		{
			...
			foreach (ArtObjectInstance levelChest in LevelChests)
			{
				...
				if (FezMath.OrientationFromDirection(FezMath.AlmostClamp(Vector3.Transform(Vector3.UnitZ, levelChest.Rotation))).AsViewpoint() != base.CameraManager.Viewpoint || levelChest.ActorSettings.ContainedTrile == ActorType.None)
				{
					continue;
				}
				chestAO = levelChest;
		...
		protected override void Begin()
		{
			...
			else
			{
				...
				TimeSpan openingDuration = OpeningDuration;
			...
			if (flag)
			{
				...
				Quaternion quaternion7 = (aoInitialRotation = (mesh.Rotation = quaternion5));
			}
			else
			{
				...
				Quaternion quaternion7 = (mesh4.Rotation = (mesh5.Rotation = quaternion5));
			}
		...
		public override void Update(GameTime gameTime)
		{
			...
			if (!base.GameState.Paused && !base.GameState.InMenuCube && !base.GameState.InMap && !base.GameState.InFpsMode && base.CameraManager.Viewpoint.IsOrthographic() && base.CameraManager.ActionRunning && !base.GameState.Loading && FloatingMaps.Count > 0)
			{
				...
				float num = (float)Math.Sin(SinceCreated * MathF.PI) * 0.1f;
		...
		protected override bool Act(TimeSpan elapsed)
		{
			...
			float num2 = sinceActive.Ticks;
			TimeSpan openingDuration = OpeningDuration;
			float num3 = FezMath.Saturate(num2 / (float)openingDuration.Ticks);
			float num4 = MathHelper.Lerp(num3, Easing.EaseInOut(num3, EasingType.Sine, EasingType.Linear), 0.5f) * (float)Math.Pow(0.5, 0.5);
			if ((double)num3 > Math.Pow(0.5, 0.5))
			{
				num4 = (float)Math.Pow(num3, 2.0);
			}
			bool flag = base.PlayerManager.Action == ActionType.OpeningTreasure;
			if (flag)
			{
				chestAO.Position = aoOrigin + num4 * 1.375f * base.CameraManager.Viewpoint.ForwardVector() + (float)Math.Sin(num4 * (MathF.PI / 2f) * 3f) * 4f / 16f * Vector3.UnitY;
				chestAO.Rotation = aoInitialRotation * Quaternion.CreateFromAxisAngle(Vector3.UnitX, (0f - num4) * (MathF.PI / 2f) * 3f / 2f);
				float num5 = num4 * (MathF.PI * 2f) + base.CameraManager.Viewpoint.ToPhi();
				base.CameraManager.Direction = new Vector3((float)Math.Sin(num5), 0f, (float)Math.Cos(num5));
				base.CameraManager.Center = aoOrigin + Vector3.UnitY * 1.5f * num3;
			}
			if (!flag)
			{
				base.PlayerManager.Velocity *= 0.95f;
			}
			lightBox.Material.Diffuse = new Vector3(FezMath.Saturate(num4 * 1.5f) * 0.75f);
			lightBox.Material.Diffuse *= new Vector3(1f, 1f, 0.5f);
			lightBox.Scale = new Vector3(1.6f, FezMath.Saturate(num4 * 1.5f) * 1.5f, 1.2f);
			Vector3 vector3;
			Vector3 vector6;
			if (num4 > 0.5f)
			{
				if (!restored && chestAO != null)
				{
					...
					{
						treasureOrigin = aoOrigin - new Vector3(0f, 0.125f, 0f);
						int num6 = IdentifierPool.FirstAvailable(base.LevelManager.ArtObjects);
						treasureAoInstance = new ArtObjectInstance(treasureAo)
						{
							Id = num6
						};
						base.LevelManager.ArtObjects.Add(num6, treasureAoInstance);
				...
				if (!flag)
				{
					num7 = ((!treasureIsMap) ? ((Easing.EaseIn((1f - num4) * 2f, EasingType.Quadratic) * 2f + 0.5f) * 2f) : ((Easing.EaseIn((1f - num4) * 2f, EasingType.Quadratic) * 4f + 0.5f) * 2f));
				}
				else if (treasureIsAo)
				{
					treasureAoInstance.Position = treasureOrigin + Vector3.UnitY * (num4 - 0.5f) * 4f;
				}
				else if (treasureIsMap)
				{
					map.Position = treasureOrigin + Vector3.UnitY * (num4 - 0.5f) * 4f;
				}
				else if (treasureIsMail)
				{
					mail.Position = treasureOrigin + Vector3.UnitY * (num4 - 0.5f) * 4f;
				}
				else
				{
					treasureInstance.Position = treasureOrigin + Vector3.UnitY * (num4 - 0.5f) * 4f;
				}
				if (treasureIsAo)
				{
					Quaternion quaternion = Quaternion.CreateFromAxisAngle(Vector3.Right, 0f - (float)Math.Asin(Math.Sqrt(2.0) / Math.Sqrt(3.0))) * Quaternion.CreateFromAxisAngle(Vector3.Up, MathF.PI / 4f);
					treasureAoInstance.Rotation = Quaternion.CreateFromAxisAngle(Vector3.Up, FezMath.WrapAngle((0f - num) * num7)) * quaternion;
				}
				else if (treasureIsMap)
				{
					if (flag)
					{
						...
						map.Rotation = Quaternion.CreateFromAxisAngle(Vector3.Up, FezMath.WrapAngle((0f - num) * num7)) * quaternion2;
					}
					else
					{
						map.Rotation = Quaternion.CreateFromAxisAngle(Vector3.Up, num7 * 1f / 30f) * map.Rotation;
					}
				}
				else if (treasureIsMail)
				{
					Quaternion quaternion3 = Quaternion.CreateFromAxisAngle(Vector3.Right, MathF.PI / 8f);
					mail.Rotation = Quaternion.CreateFromAxisAngle(Vector3.Up, FezMath.WrapAngle((0f - num) * num7)) * quaternion3;
				}
				else
				{
					treasureInstance.Phi = FezMath.WrapAngle((0f - num) * num7);
				...
				vector6 = (mesh.Position = (mesh2.Position = vector3));
				if (treasureIsAo)
				{
			...
			float num8 = FezMath.Saturate((num4 - 0.95f) * 19.999996f);
			fadedStar.Material.Opacity = ((treasureIsMail || treasureIsMap || treasureIsAo || treasureTrile.ActorSettings.Type.IsCubeShard()) ? num8 : 0f);
			flare.Material.Diffuse = new Vector3(num8 / 3f);
			Vector3 vector7 = MathHelper.Lerp(base.CameraManager.Radius * 0.6f, base.CameraManager.Radius * 0.5f, (float)Math.Sin(sinceActive.TotalSeconds * 5.0) / 2f + 0.5f) * Vector3.One;
			Mesh mesh3 = flare;
			Mesh mesh4 = solidStar;
			vector3 = (fadedStar.Scale = vector7 * num8);
			vector6 = (mesh3.Scale = (mesh4.Scale = vector3));
			Quaternion quaternion6 = (solidStar.Rotation = (fadedStar.Rotation = base.CameraManager.Rotation * Quaternion.CreateFromAxisAngle(Vector3.UnitZ, num * 1f)));
			if (base.LevelManager.WaterType == LiquidType.Sewer)
			{
				lightBox.Material.Diffuse *= new Vector3(58f / 85f, 0.76862746f, 0.2509804f);
				flare.Material.Diffuse = new Vector3(58f / 85f, 0.76862746f, 0.2509804f) * new Vector3(num8 / 3f);
				flare.Scale *= 0.75f;
			...
			else if (base.LevelManager.WaterType == LiquidType.Lava)
			{
				...
				flare.Material.Diffuse = new Vector3(0.99607843f, 0.003921569f, 0f) * num8 / 2.5f;
				flare.Scale *= 0.75f;
				solidStar.Enabled = true;
				solidStar.Material.Opacity = fadedStar.Material.Opacity * 0.75f;
				solidStar.Scale = base.CameraManager.Radius * Vector3.One;
