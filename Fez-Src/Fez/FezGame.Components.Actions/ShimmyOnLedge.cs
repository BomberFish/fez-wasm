...
namespace FezGame.Components.Actions
{
	public class ShimmyOnLedge : PlayerAction
	{
		...
		protected override void TestConditions()
		{
			...
			if ((uint)(actionType - 73) <= 1u && base.InputManager.Movement.X != 0f && base.PlayerManager.Animation.Timing.Ended)
			{
		...
		protected override bool Act(TimeSpan elapsed)
		{
			...
			bool flag = base.PlayerManager.HeldInstance == null;
			Vector3 vector = base.CameraManager.Viewpoint.ForwardVector() * ((!base.PlayerManager.Background) ? 1 : (-1));
			bool flag2 = base.PlayerManager.HeldInstance != null && base.PlayerManager.HeldInstance.GetRotatedFace(base.CameraManager.Viewpoint.VisibleOrientation()) == CollisionType.None;
			if (flag || (flag2 && base.PlayerManager.HeldInstance.Position.Dot(vector) > heldInstance.Position.Dot(vector)))
			{
			...
			float num2 = base.InputManager.Movement.X * 4.7f * 0.15f * (float)elapsed.TotalSeconds;
			if (base.PlayerManager.Action != ActionType.ShimmyBack && base.PlayerManager.Action != ActionType.ShimmyFront)
			{
				num2 *= 0.6f;
			}
			base.PlayerManager.Velocity = num2 * base.CameraManager.Viewpoint.RightVector() * (1f + num);
		...
		protected override bool IsActionAllowed(ActionType type)
		{
			...
			case ActionType.GrabLedgeBack:
				result = ((base.InputManager.Movement.X != 0f) ? 1 : 0);
				break;
			default:
				result = 0;
				break;
