	...
	public class Teeter : PlayerAction
	{
		...
		protected override void TestConditions()
		{
			...
			case ActionType.IdleYawn:
			{
				...
				float num = nearLow.Center.Dot(b);
				float num2 = Math.Abs(num - base.PlayerManager.Position.Dot(b));
				if (!(num2 > 1f) && num2 > 0.45f)
				{
					MultipleHits<CollisionResult> result = base.CollisionManager.CollideEdge(nearLow.Center, Vector3.Down * Math.Sign(base.CollisionManager.GravityFactor), base.PlayerManager.Size * FezMath.XZMask / 2f, Direction2D.Vertical);
					if (!result.AnyHit())
					{
