	...
	public class WalkRun : PlayerAction
	{
		...
		protected override bool Act(TimeSpan elapsed)
		{
			...
			else if (base.PlayerManager.Action != ActionType.Landing)
			{
				...
				{
					bool flag = base.PlayerManager.Action == ActionType.Walking;
					base.PlayerManager.Action = ActionType.Running;
				...
				{
					base.PlayerManager.Action = ActionType.Walking;
					num = Easing.EaseOut(Math.Min(1f, Math.Abs(base.InputManager.Movement.X) * 2f), EasingType.Cubic);
				}
				base.PlayerManager.Animation.Timing.Update(elapsed, num * (1f + Math.Abs(base.CollisionManager.GravityFactor)) / 2f);
			}
