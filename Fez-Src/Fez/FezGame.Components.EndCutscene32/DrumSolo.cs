...
namespace FezGame.Components.EndCutscene32
{
	internal class DrumSolo : DrawableGameComponent
	{
		...
		public override void Initialize()
		{
			...
			for (int i = 0; i < 8; i++)
			{
				float num2 = (float)i * (MathF.PI * 2f) / 8f;
				float num3 = ((float)i + 0.5f) * (MathF.PI * 2f) / 8f;
		...
		public override void Update(GameTime gameTime)
		{
			...
			case State.DrumSolo:
			{
				...
				Vector3 vector3 = (Flare.Position = (StarMesh.Position = PlayerManager.Position + Vector3.Normalize(-CameraManager.Direction) * 10f));
				Vector3 vector9 = MathHelper.Lerp(CameraManager.Radius * 0.8f, CameraManager.Radius * 0.6f, (float)Math.Sin(Time * 5f) / 2f + 0.5f) * Vector3.One;
				vector3 = (Flare.Scale = (StarMesh.Scale = vector9 * num7));
			...
			case State.JumpUp:
			{
				...
				{
					Vector3 vector3 = (Flare.Position = (StarMesh.Position = PlayerManager.Position + Vector3.Normalize(-CameraManager.Direction) * 10f));
					Vector3 vector4 = MathHelper.Lerp(CameraManager.Radius * 0.6f, CameraManager.Radius * 0.4f, (float)Math.Sin(Time * 5f) / 2f + 0.5f) * Vector3.One;
					vector3 = (Flare.Scale = (StarMesh.Scale = vector4 * num4));
					StarRotationTimeKeeper += num * (float)Math.Sin(Time) * 0.75f + 1f;
