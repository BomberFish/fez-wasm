...
namespace FezGame.Components.EndCutscene32
{
	internal class FezGrid : DrawableGameComponent
	{
		...
		private void Reset()
		{
			...
			Quaternion quaternion2 = (FezGroup.Rotation = (GomezGroup.Rotation = Quaternion.Identity));
		...
		public override void Update(GameTime gameTime)
		{
			...
			case State.RotateZoom:
			{
				...
				float amount2 = num3;
				GomezGroup.Rotation = Quaternion.Slerp(Quaternion.Identity, Quaternion.CreateFromAxisAngle(Vector3.UnitZ, MathF.PI / 4f), amount2);
				FezGroup.Rotation = Quaternion.Slerp(Quaternion.Identity, Quaternion.CreateFromAxisAngle(Vector3.UnitZ, MathF.PI / 2f), amount2);
			...
			case State.GridFadeAlign:
			{
				...
				TetraMesh.Effect.ForcedProjectionMatrix = Matrix.CreatePerspectiveFieldOfView(MathHelper.Lerp(MathF.PI / 2f, MathF.PI * 3f / 4f, amount3), CameraManager.AspectRatio, 0.1f, 2000f);
				...
				TetraMesh.Effect.ForcedViewMatrix = Matrix.CreateLookAt(vector2, Vector3.Lerp(vector2 - Vector3.UnitY, vector2 + Vector3.UnitX - Vector3.UnitY, amount5), FezMath.Slerp(new Vector3((float)Math.Sin(num4 * MathF.PI * 0.7f), 0f, (float)Math.Cos(num4 * MathF.PI * 0.7f)), Vector3.UnitY, step));
				if (Time > 12f)
				{
					ChangeState();
				}
			...
			case State.CubeRise:
			{
				...
				TetraMesh.Effect.ForcedProjectionMatrix = Matrix.CreatePerspectiveFieldOfView(MathF.PI * 3f / 4f, CameraManager.AspectRatio, 0.1f, 2000f);
