	...
	internal class VibratingMembrane : DrawableGameComponent
	{
		...
		public override void Update(GameTime gameTime)
		{
			...
			case State.GridFadeAlign:
			{
				...
				Mesh mesh = ((num2 < 0.25f) ? LinesMesh : VibratingMesh);
				mesh.Effect.ForcedProjectionMatrix = Matrix.CreatePerspectiveFieldOfView(MathHelper.Lerp(MathF.PI / 2f, MathF.PI * 3f / 4f, amount), CameraManager.AspectRatio, 0.1f, 2000f);
				...
				mesh.Material.Opacity = Easing.EaseIn(FezMath.Saturate(num2 * 2.5f), EasingType.Sine);
				Vector3 vector = Vector3.Lerp(Vector3.UnitY * 360f, Vector3.UnitY * 1.5f, amount2) + Vector3.UnitX * num2 * 100f;
				mesh.Effect.ForcedViewMatrix = Matrix.CreateLookAt(vector, Vector3.Lerp(vector - Vector3.UnitY, vector + Vector3.UnitX - Vector3.UnitY * 3f, amount3), FezMath.Slerp(new Vector3((float)Math.Sin(num2 * MathF.PI * 0.7f), 0f, (float)Math.Cos(num2 * MathF.PI * 0.7f)), Vector3.UnitY, step));
				float num5 = (VibratingEffect.FogDensity = (StaticEffect.FogDensity = MathHelper.Lerp(0.0025f, 0.1f, Easing.EaseIn(num2, EasingType.Cubic))));
			...
			case State.PanDown:
			{
				float num7 = FezMath.Saturate(Time / 6f);
				VibratingEffect.TimeStep += num;
				VibratingEffect.FogDensity = 0.1f;
				VibratingEffect.Intensity = FezMath.Saturate(num7 / 2f + 0.5f);
				VibratingMesh.Effect.ForcedProjectionMatrix = Matrix.CreatePerspectiveFieldOfView(MathF.PI * 3f / 4f, CameraManager.AspectRatio, 0.1f, 2000f);
				Vector3 vector6 = Vector3.UnitY * 1.5f + Vector3.UnitX * num7 * 50f;
				VibratingMesh.Effect.ForcedViewMatrix = Matrix.CreateLookAt(vector6, vector6 + Vector3.UnitX - Vector3.UnitY * MathHelper.Lerp(3f, 0f, Easing.EaseInOut(num7, EasingType.Quadratic)), Vector3.Up);
				if (Time > 6f)
				{
					ChangeState();
				}
			...
			case State.Rotate:
			{
				float num8 = FezMath.Saturate(Time / 6f);
				VibratingEffect.TimeStep += num;
				VibratingEffect.Intensity = 1f;
				float step2 = Easing.EaseInOut(num8, EasingType.Quadratic);
				Vector3 vector7 = Vector3.UnitY * 1.5f + Vector3.UnitX * (num8 * 50f + 50f);
				VibratingMesh.Effect.ForcedViewMatrix = Matrix.CreateLookAt(vector7, vector7 + Vector3.UnitX, FezMath.Slerp(Vector3.Up, -Vector3.UnitZ, step2));
				if (Time > 6f)
				{
					ChangeState();
				}
			...
			case State.CubeReveal:
			{
				if (Time == 0f)
				{
					...
					Vector3 vector4 = (PointsMesh.Scale = (CubeMesh.Scale = new Vector3(0.001f, 0.001f, 0.001f)));
				...
				VibratingEffect.TimeStep += num;
				VibratingEffect.Intensity = MathHelper.Lerp(1f, 5f, Easing.EaseIn(num6, EasingType.Quadratic));
				VibratingEffect.FogDensity = MathHelper.Lerp(0.1f, 1f, Easing.EaseIn(num6, EasingType.Cubic));
				GridsCameraDistance = MathHelper.Lerp(1f, 10f, Easing.EaseIn(num6, EasingType.Quadratic));
				...
				if (num6 > 0.5f)
				{
					CubeMesh.Rotation = Quaternion.CreateFromAxisAngle(Vector3.UnitZ, 0.0025f * ((num6 - 0.5f) * 2f)) * CubeMesh.Rotation;
				}
				PointsMesh.Position = CubeMesh.Position;
				PointsMesh.Scale = CubeMesh.Scale;
				PointsMesh.Rotation = CubeMesh.Rotation;
				Vector3 vector5 = Vector3.UnitY * 1.5f * GridsCameraDistance + Vector3.UnitX * (num6 * 50f + 100f);
				VibratingMesh.Effect.ForcedViewMatrix = Matrix.CreateLookAt(vector5, vector5 + Vector3.UnitX, -Vector3.UnitZ);
				if (Time > 10f)
				{
					ChangeState();
				}
		...
		public override void Draw(GameTime gameTime)
		{
			...
			case State.GridFadeAlign:
			{
				float num = FezMath.Saturate(Time / 12f);
				if (num < 0.25f)
				{
					LinesMesh.Draw();
				}
			...
			case State.Rotate:
				...
				VibratingMesh.Draw();
				VibratingMesh.Rotation = Quaternion.CreateFromAxisAngle(Vector3.UnitX, MathF.PI);
