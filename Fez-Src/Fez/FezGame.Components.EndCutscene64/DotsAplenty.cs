	...
	internal class DotsAplenty : DrawableGameComponent
	{
		...
		private void MakeDot()
		{
			...
			for (int i = -10; i <= 10; i++)
			{
				for (int j = -10; j <= 10; j++)
				{
					Group group = CloneMesh.AddFace(new Vector3(600f, 600f / aspectRatio, 1f), Vector3.Zero, FaceOrientation.Front, centeredOnOrigin: true);
					group.Position = new Vector3(j * 575, (float)(i * 575) / aspectRatio, 0f);
				}
			}
			...
			Group group2 = DotMesh.AddGroup();
			ShaderInstancedIndexedPrimitives<VertexPosition4ColorInstance, Vector4> shaderInstancedIndexedPrimitives = (ShaderInstancedIndexedPrimitives<VertexPosition4ColorInstance, Vector4>)(group2.Geometry = new ShaderInstancedIndexedPrimitives<VertexPosition4ColorInstance, Vector4>(PrimitiveType.TriangleList, 100));
			...
			for (int n = -num4; n < num4; n++)
			{
				for (int num6 = -num5; num6 < num5; num6++)
				{
					...
					shaderInstancedIndexedPrimitives.Instances[num7] = new Vector4(num6 * 6, n * 6 + ((Math.Abs(num6) % 2 != 0) ? 3 : 0), 0f, (num6 == 0 && n == 0) ? 0f : ((float)random.NextDouble() * MathF.PI * 2f));
				}
			}
