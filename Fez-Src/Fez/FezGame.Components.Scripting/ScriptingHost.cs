...
namespace FezGame.Components.Scripting
{
	internal class ScriptingHost : GameComponent, IScriptingManager
	{
		...
		public static bool ScriptExecuted = false;
		...
		{
			if ((GameState.Loading && trigger.Object.Type != "Level" && trigger.Event != "Start") || script.Disabled || id != trigger.Object.Identifier || (script.Conditions != null && script.Conditions.Any((ScriptCondition c) => !c.Check(services[c.Object.Type]))) || (script.OneTime && activeScripts.Any((ActiveScript x) => x.Script == script)))
			{
				return;
			}
