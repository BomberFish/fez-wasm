	...
	internal class BitDoorState
	{
		...
		public void Update(TimeSpan elapsed)
		{
			...
			if (!AoInstance.ActorSettings.Inactive)
			{
				...
				if (num >= AoInstance.ArtObject.ActorType.GetBitCount() && sinceClose.TotalSeconds > 0.5)
				{
					OpenDoor(elapsed);
				}
			}
		...
		private void OpenDoor(TimeSpan elapsed)
		{
			...
			{
				long ticks = sinceMoving.Ticks;
				TimeSpan doorShakeTime = DoorShakeTime;
				float num = ticks - doorShakeTime.Ticks;
				doorShakeTime = DoorOpenTime;
				float amount = FezMath.Saturate(Easing.EaseInOut(num / (float)doorShakeTime.Ticks, EasingType.Sine));
