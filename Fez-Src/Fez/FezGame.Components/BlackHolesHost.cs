	...
	public class BlackHolesHost : DrawableGameComponent, IBlackHoleManager
	{
		private class HoleState
		{
			...
			private void BuildMesh()
			{
				...
				FaceOrientation[] array2 = array;
				foreach (FaceOrientation faceOrientation in array2)
				{
					...
					{
						Vector3 p = vector6 + k * Vector3.UnitY;
				...
				foreach (Group group3 in mesh.Groups)
				{
					group3.TextureMatrix = new Matrix(0.5f, 0f, 0f, 0f, 0f, 0.5f, 0f, 0f, (0.5 > Host.random.NextDouble()) ? 0.5f : 0f, (0.5 > Host.random.NextDouble()) ? 0.5f : 0f, 1f, 0f, 0f, 0f, 0f, 1f);
				}
				...
				Group group = Host.HolesBodyMesh.AddGroup();
				group.Geometry = new IndexedUserPrimitives<VertexPositionInstance>(indexedUserPrimitives.Vertices.Select((VertexPositionNormalColor x) => new VertexPositionInstance(x.Position)
				{
					InstanceIndex = instanceIndex
				}).ToArray(), indexedUserPrimitives.Indices, PrimitiveType.TriangleList);
				Group group2 = Host.HolesFringeMesh.AddGroup();
				group2.Geometry = new IndexedUserPrimitives<VertexPositionTextureInstance>(indexedUserPrimitives2.Vertices.Select((FezVertexPositionNormalTexture x) => new VertexPositionTextureInstance(x.Position, x.TextureCoordinate)
				{
					InstanceIndex = instanceIndex
		...
		public override void Update(GameTime gameTime)
		{
			...
			foreach (HoleState hole2 in holes)
			{
				if (hole2.Visible && hole2.Enabled)
				{
					...
					double num = (hole2.Sucking ? 0.2 : (gameTime.ElapsedGameTime.TotalSeconds * 0.5));
					bool flag = num > random.NextDouble();
					...
					float num2 = (float)random.NextDouble() * 16f;
					float num3 = 3f / 32f * (flag ? num2 : 1f) * 2f;
					for (int i = 0; i < 3; i++)
					{
						hole2.Offsets[i].X = (float)(random.NextDouble() - 0.5) * num3;
						hole2.Offsets[i].Y = (float)(random.NextDouble() - 0.5) * num3;
						hole2.Offsets[i].Z = (float)(random.NextDouble() - 0.5) * num3;
					}
				}
			}
		}
		...
		public void DoDraw()
		{
			...
			for (int i = 0; i < 3; i++)
			{
				graphicsDevice.SetColorWriteChannels((i switch
				{
					1 => ColorWriteChannels.Green, 
					0 => ColorWriteChannels.Red, 
					_ => ColorWriteChannels.Blue, 
