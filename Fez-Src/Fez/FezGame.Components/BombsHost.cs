...
namespace FezGame.Components
{
	internal class BombsHost : DrawableGameComponent
	{
		...
		public override void Update(GameTime gameTime)
		{
			...
			foreach (TrileInstance key in bombStates.Keys)
			{
				...
				if (bombState.SincePickup > FlashTime && bombState.Explosion == null)
				{
					...
					double num2 = num;
					TimeSpan explodeStart = ExplodeStart;
					if (num2 > explodeStart.TotalSeconds - 1.0)
					{
						num *= 2.0;
					}
				...
				if (bombState.SincePickup > ExplodeStart && bombState.Explosion == null)
				{
					...
					bombState.Explosion.OriginalRotation = Quaternion.CreateFromAxisAngle(Vector3.UnitZ, (float)RandomHelper.Random.Next(0, 4) * (MathF.PI / 2f));
					...
					Vector3 vector3 = (bombState.Explosion.Position = (bombState.Flare.Position = key.Center + (-0.5f + RandomHelper.Centered(0.0010000000474974513)) * CameraManager.Viewpoint.ForwardVector()));
					float num3 = (flag5 ? 3f : 1.5f);
					float num4 = ((PlayerManager.Position - key.Center) * CameraManager.Viewpoint.ScreenSpaceMask()).Length();
					if ((PlayerManager.CarriedInstance == key || num4 < num3) && PlayerManager.Action != ActionType.Dying)
					{
		...
		private void DestroyNeighborhood(TrileInstance instance, BombState state)
		{
			...
			Point[] array = ((instance.Trile.ActorSettings.Type == ActorType.BigBomb) ? BigBombOffsets : SmallBombOffsets);
