...
namespace FezGame.Components
{
	internal class ClockTowerHost : GameComponent
	{
		...
		private void TryInitialize()
		{
			...
			if (RedAo.Enabled)
			{
				float angle = (lastRedAngle = FezMath.WrapAngle((float)FezMath.Round(timeSpan.TotalSeconds) / 60f * (MathF.PI * 2f)));
		...
		public override void Update(GameTime gameTime)
		{
			if (!GameState.Paused && !GameState.InMap && CameraManager.ActionRunning && CameraManager.Viewpoint.IsOrthographic() && !GameState.Loading && PlayerManager.Action != ActionType.FindingTreasure && !GameState.FarawaySettings.InTransition)
			{
				...
				if (RedAo.Enabled)
				{
					float num = (lastRedAngle = FezMath.CurveAngle(lastRedAngle, FezMath.WrapAngle((float)FezMath.Round(timeSpan.TotalSeconds) / 60f * (MathF.PI * 2f)), 0.1f));
				...
				if (BlueAo.Enabled)
				{
					float num2 = FezMath.WrapAngle((float)timeSpan.TotalMinutes / 60f * (MathF.PI * 2f));
					...
					BlueSecret = TestSecretFor(FezMath.AlmostEqual(num2, MathF.PI / 2f, 0.125f), BlueAo, BlueSecret, BlueTopMost);
				}
				if (GreenAo.Enabled)
				{
					float num3 = FezMath.WrapAngle((float)timeSpan.TotalHours / 24f * (MathF.PI * 2f));
					...
					GreenSecret = TestSecretFor(FezMath.AlmostEqual(num3, MathF.PI / 2f, 0.125f), GreenAo, GreenSecret, GreenTopMost);
				}
				if (WhiteAo.Enabled)
				{
					float num4 = FezMath.WrapAngle((float)timeSpan.TotalDays / 7f * (MathF.PI * 2f));
					...
					WhiteSecret = TestSecretFor(FezMath.AlmostEqual(num4, MathF.PI / 2f, 0.125f), WhiteAo, WhiteSecret, WhiteTopMost);
				}
			}
		}
