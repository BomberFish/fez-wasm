...
namespace FezGame.Components
{
	public class DotHost : DrawableGameComponent, IDotManager
	{
		...
		public bool Hidden
		{
			get
			{
				return !base.Visible && !base.Enabled;
			}
			set
			{
		...
		public override void Initialize()
		{
			...
			Group group = DotMesh.AddGroup();
			group.Geometry = (DotWireGeometry = new IndexedUserPrimitives<FezVertexPositionColor>(PrimitiveType.LineList));
			group = DotMesh.AddGroup();
			group.Geometry = (DotFacesGeometry = new IndexedUserPrimitives<FezVertexPositionColor>(PrimitiveType.TriangleList));
		...
		private void Update(GameTime gameTime, bool force)
		{
			...
			if (base.Visible)
			{
				...
				{
					Vector4 vector = Vector4.Transform(Vertices[i], matrix2);
			...
			if (base.Visible)
			{
				DotMesh.Rotation = CamRotationFollow * Quaternion.CreateFromAxisAngle(Vector3.Right, (float)Math.Asin(Math.Sqrt(2.0) / Math.Sqrt(3.0))) * Quaternion.CreateFromAxisAngle(Vector3.Up, MathF.PI / 4f);
			}
		...
		private void UpdateRays(float elapsedSeconds)
		{
			...
			for (int num4 = RaysMesh.Groups.Count - 1; num4 >= 0; num4--)
			{
				...
				float num5 = (float)Math.Sin(rayState.Age * (MathF.PI * 2f) - MathF.PI / 2f) * 0.5f + 0.5f;
			...
			Vector3 vector2 = (FlareMesh.Position = (RaysMesh.Position = DotMesh.Position));
		...
		public override void Draw(GameTime gameTime)
		{
			...
			bool flag = Behaviour == BehaviourType.ThoughtBubble;
			FlareMesh.Draw();
			...
			if (flag && (FaceButton == DotFaceButton.Up || DestinationVignette != null))
			{
			...
			if (!flag || (FaceButton != 0 && DestinationVignette == null))
			{
				...
				DotMesh.Material.Opacity = 1f;
				DotMesh.Material.Diffuse = new Vector3(AlwaysShowLines ? Opacity : (num * 0.5f * Opacity));
				DotMesh.Draw();
