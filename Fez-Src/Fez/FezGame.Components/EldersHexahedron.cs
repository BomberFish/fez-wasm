...
namespace FezGame.Components
{
	internal class EldersHexahedron : DrawableGameComponent
	{
		...
		public override void Initialize()
		{
			...
			LoadSounds();
			AoInstance.Rotation = Quaternion.CreateFromAxisAngle(Vector3.Up, MathF.PI / 2f) * Quaternion.CreateFromAxisAngle(Vector3.Right, (float)Math.Asin(Math.Sqrt(2.0) / Math.Sqrt(3.0))) * Quaternion.CreateFromAxisAngle(Vector3.Up, MathF.PI / 4f);
			Origin = AoInstance.Position;
			AoRotationOrigin = Quaternion.CreateFromAxisAngle(Vector3.Forward, MathF.PI / 4f) * Quaternion.CreateFromAxisAngle(Vector3.Up, MathF.PI / 4f);
		...
		private void GenerateCubes()
		{
			...
			Quaternion quaternion2 = (SmallCubes.Rotation = (SolidCubes.Rotation = AoRotationOrigin));
			...
			foreach (Vector3 vector in array2)
			{
				...
				group.Rotation = Quaternion.CreateFromAxisAngle(Vector3.Up, (float)RandomHelper.Random.Next(0, 4) * (MathF.PI / 2f));
				...
				group = SmallCubes.AddGroup();
				group.Geometry = new IndexedUserPrimitives<VertexPositionNormalTextureInstance>(trile2.Geometry.Vertices.ToArray(), trile2.Geometry.Indices, trile2.Geometry.PrimitiveType);
				group.Position = vector * RandomHelper.Between(0.5, 1.0);
				group.Rotation = Quaternion.CreateFromAxisAngle(Vector3.Up, (float)RandomHelper.Random.Next(0, 4) * (MathF.PI / 2f));
				group.BakeTransformWithNormal<VertexPositionNormalTextureInstance>();
				group.CustomData = new ShardProjectionData
				{
			...
			DrawActionScheduler.Schedule(delegate
			{
				...
				Dirtyable<Texture> dirtyable3 = (SolidCubes.Texture = (SmallCubes.Texture = LevelManager.TrileSet.TextureAtlas));
			});
		}
		...
		public override void Update(GameTime gameTime)
		{
			...
			case Phase.MatrixSpin:
			{
				...
				CameraManager.Direction = Vector3.Normalize(new Vector3((float)Math.Sin(OriginalSpin + CameraSpins * (MathF.PI * 2f)), 0f, (float)Math.Cos(OriginalSpin + CameraSpins * (MathF.PI * 2f))));
				CameraManager.SnapInterpolation();
				BeamMesh.Position = new Vector3(Origin.X, (AoInstance.Position.Y + 26f) / 2f, Origin.Z) + CameraManager.InverseView.Forward * 5f;
				BeamMesh.Rotation = CameraManager.Rotation * Quaternion.CreateFromAxisAngle(Vector3.Up, -MathF.PI / 2f);
		...
		private void HexaExplode(float elapsedTime)
		{
			AoInstance.Hidden = true;
			AoInstance.Visible = false;
			Vector3 vector2 = (SmallCubes.Position = (SolidCubes.Position = AoInstance.Position));
			if (SincePhaseStarted > 0.25f)
			{
				float num = SincePhaseStarted / 13f;
				if (sExplode != null)
				{
					sExplode.Emit();
					sExplode = null;
			...
			{
				for (int num2 = TrialRaysMesh.Groups.Count - 1; num2 >= 0; num2--)
				{
					Group group = TrialRaysMesh.Groups[num2];
					...
					{
						TrialRaysMesh.RemoveGroupAt(num2);
					}
				}
			}
		...
		private void UpdateRays(float elapsedSeconds)
		{
			if (GameState.IsTrialMode)
			{
				...
				for (int num3 = TrialRaysMesh.Groups.Count - 1; num3 >= 0; num3--)
				{
					...
					float num4 = (float)Math.Sin(rayState.Age * (MathF.PI * 2f) - MathF.PI / 2f) * 0.5f + 0.5f;
				...
				Vector3 vector2 = (TrialFlareMesh.Position = (TrialRaysMesh.Position = AoInstance.Position));
				Quaternion quaternion2 = (TrialFlareMesh.Rotation = (TrialRaysMesh.Rotation = CameraManager.Rotation));
		...
		{
			ScreenFade component = new ScreenFade(base.Game)
			{
		...
		{
			IWaiter waiter = Waiters.Wait(() => PlayerManager.Grounded, delegate
			{
		...
		public override void Draw(GameTime gameTime)
		{
			...
			if (CurrentPhase > Phase.Talk1)
			{
				...
				foreach (BackgroundPlane backgroundPlane in statuePlanes)
				{
					backgroundPlane.Opacity = Easing.EaseIn(1f - Starfield.Opacity, EasingType.Quadratic);
				}
