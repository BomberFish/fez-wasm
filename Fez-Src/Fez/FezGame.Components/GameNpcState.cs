...
namespace FezGame.Components
{
	public class GameNpcState : NpcState
	{
		...
		protected override float AnimationSpeed => (CurrentAction == NpcAction.Walk || CurrentAction == NpcAction.Turn) ? (Npc.WalkSpeed / ((originalSpeed == 0f) ? 1f : originalSpeed)) : 1f;
		...
		protected override void TryFlee()
		{
			...
			float num2 = a.Dot(base.CameraManager.Viewpoint.RightVector());
		...
		{
			ActionType action = PlayerManager.Action;
			ActionType actionType = action;
			if ((actionType != ActionType.Idle && (uint)(actionType - 6) > 1u && actionType != ActionType.Sliding) || PlayerManager.Background || !SpeechManager.Hidden || (Npc.ActorType == ActorType.Owl && (OwlInvisible || CurrentAction == NpcAction.TakeOff || CurrentAction == NpcAction.Fly)))
			{
				return;
			}
		...
		private bool TestObstruction(Vector3 hitStart, float hitDistance)
		{
			...
			foreach (TrileInstance value in base.LevelManager.Triles.Values)
			{
				if (value.InstanceId != -1 && ((value.Center - hitStart) * vector2).LengthSquared() < 0.5f)
				{
					...
					{
						Vector3 vector3 = value.Position + Vector3.One / 2f + vector * -0.5f;
						float num = (vector3 - hitStart).Dot(vector);
						return num <= hitDistance + 0.25f;
					}
				}
			}
			return false;
		...
		private void Talk()
		{
			...
			if (Math.Abs(value) < 1f && !wasTalking)
			{
				...
				MultipleHits<TrileInstance> ground = PlayerManager.Ground;
