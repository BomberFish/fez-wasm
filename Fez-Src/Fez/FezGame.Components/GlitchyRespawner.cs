		...
		public override void Initialize()
		{
			...
			lock (StaticLock)
			{
				...
				if (CubemappedEffect == null)
				{
					DrawActionScheduler.Schedule(delegate
					{
						SpawnMesh.Effect = (CubemappedEffect = new CubemappedEffect());
					});
				}
			}
			...
			{
				Instance.Phi = (float)FezMath.Round(Instance.Phi / (MathF.PI / 2f)) * (MathF.PI / 2f);
			}
			...
			{
				group.Rotation = Quaternion.CreateFromAxisAngle(Vector3.Left, (float)Math.Asin(Math.Sqrt(2.0) / Math.Sqrt(3.0))) * Quaternion.CreateFromAxisAngle(Vector3.Down, MathF.PI / 4f) * group.Rotation;
			}
