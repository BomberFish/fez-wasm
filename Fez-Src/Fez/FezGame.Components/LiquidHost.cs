	...
	public class LiquidHost : DrawableGameComponent
	{
		...
		private void CreateFoam()
		{
			...
			case LiquidType.Green:
			{
				...
				group2 = FoamMesh.CollapseToBuffer<VertexPositionNormalColor>(1, FoamMesh.Groups.Count - 1);
				group2.Material = new Material();
				group2.Position = new Vector3(0f, -1f, 0f);
				group2.CustomData = false;
				break;
			}
			}
		}
		...
		private void CreateParticleSystems()
		{
			...
			LiquidType liquidType = waterType;
			if ((uint)(liquidType - 3) <= 1u)
			{
		...
		private void SpawnBubbles(TimeSpan elapsed, float waterLevel, bool invisible)
		{
			...
			if (!invisible)
			{
				...
				{
					Position = position,
					Rotation = CameraRotation * (RandomHelper.Probability(0.5) ? Quaternion.CreateFromAxisAngle(Vector3.Up, MathF.PI) : Quaternion.Identity),
					Doublesided = true,
					Loop = false,
					Timing = 
					{
