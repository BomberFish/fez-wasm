	...
	public class LoadingScreen : DrawableGameComponent
	{
		...
		protected override void LoadContent()
		{
			...
			mesh = new Mesh
			{
				...
				AlwaysOnTop = false,
				DepthWrites = true,
				Rotation = Quaternion.CreateFromAxisAngle(Vector3.Right, (float)Math.Asin(Math.Sqrt(2.0) / Math.Sqrt(3.0))) * Quaternion.CreateFromAxisAngle(Vector3.Up, MathF.PI / 4f)
			};
