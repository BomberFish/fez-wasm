...
namespace FezGame.Components
{
	internal class MenuCube : DrawableGameComponent
	{
		...
		private MenuCubeFace Face = MenuCubeFace.CubeShards;

		private MenuCubeFace LastFace = MenuCubeFace.CubeShards;
		...
		public override void Initialize()
		{
			...
			if (flag)
			{
				ArtObjectMaterializer artObjectMaterializer = new ArtObjectMaterializer(artObject);
				artObjectMaterializer.RecomputeTexCoords(widen: false);
			}
			...
			Vector3 vector6 = (antiCubes.Position = (maps.Position = vector3));
			Mesh antiCubes2 = AntiCubes;
			Mesh maps2 = Maps;
			Mesh hidingPlanes2 = HidingPlanes;
			Mesh goldenCubes = GoldenCubes;
			Vector3 vector8 = (AoInstance.Scale = new Vector3(0f));
			vector = (goldenCubes.Scale = vector8);
			vector3 = (hidingPlanes2.Scale = vector);
			vector6 = (antiCubes2.Scale = (maps2.Scale = vector3));
			TransformArtifacts();
		...
		private void CreateTomePages()
		{
			...
			for (int i = 0; i < 2; i++)
			{
				...
				{
					int num = i * 4 + j;
					Group group = TomePages.AddFace(size, origin, FaceOrientation.Front, centeredOnOrigin: false);
					group.TextureMatrix = new Matrix(0.25f, 0f, 0f, 0f, 0f, 0.25f, 0f, 0f, (float)i / 2f, (float)j / 4f, 1f, 0f, 0f, 0f, 0f, 0f);
					group = TomePages.AddFace(size, origin, FaceOrientation.Back, centeredOnOrigin: false);
					group.TextureMatrix = new Matrix(0.25f, 0f, 0f, 0f, 0f, 0.25f, 0f, 0f, ((float)i + 0.5f) / 2f, (float)j / 4f, 1f, 0f, 0f, 0f, 0f, 0f);
		...
		private void FillInPlanes()
		{
			...
			foreach (MenuCubeFace value in Util.GetValues<MenuCubeFace>())
			{
				...
				for (int i = 0; i < value.GetCount() && i < 32; i++)
				{
					...
					int num4 = num2 % num3;
					int num5 = num2 / num3;
					Vector3 vector17 = (group.Position = (float)(num4 * value.GetSpacing()) / 16f * value.GetRight() + (float)(num5 * value.GetSpacing()) / 16f * -Vector3.UnitY + size / 2f * (value.GetForward() + Vector3.Up - value.GetRight()) + value.GetForward() * -8f / 16f + (Vector3.Down + value.GetRight()) * value.GetOffset() / 16f);
				}
				Group group2 = HidingPlanes.CollapseToBufferWithNormal<VertexPositionNormalColor>(num, HidingPlanes.Groups.Count - num);
				group2.CustomData = value;
				num++;
			}
		}
		...
		private void CreateMapsFace()
		{
			...
			Quaternion quaternion = Quaternion.CreateFromAxisAngle(Vector3.Up, -MathF.PI / 2f);
			int num = 0;
			foreach (string map in GameState.SaveData.Maps)
			{
				...
				group.Rotation = Quaternion.CreateFromAxisAngle(Vector3.Up, MathF.PI / 8f) * quaternion;
		...
		private void CreateAntiCubeFace()
		{
			...
			else
			{
				...
				for (int i = 0; i < GameState.SaveData.SecretCubes; i++)
				{
					...
					group.Scale = new Vector3(0.5f);
					group.Rotation = Quaternion.CreateFromAxisAngle(Vector3.Up, MathF.PI / 2f * (float)RandomHelper.Random.Next(0, 4));
				}
		...
		private void CreateFaceHighlights(MenuCubeFace cf, Color color)
		{
			...
			for (int i = 0; i < 4; i++)
			{
				Group group = Highlights.AddWireframeFace(new Vector3((float)cf.GetSize() * 1.25f / 16f) * ((Vector3.UnitY + cf.GetRight().Abs()) * (0.95f - (float)i * 0.05f)), Vector3.Zero, FezMath.OrientationFromDirection(cf.GetForward()), new Color(vector.X, vector.Y, vector.Z, 1f - (float)i / 4f), centeredOnOrigin: true);
				group.Position = size / 2f * (cf.GetForward() + Vector3.Up - cf.GetRight()) + cf.GetForward() * -0.4375f + (Vector3.Down + cf.GetRight()) * cf.GetOffset() / 16f;
			}
		}
		...
		private void StartInTransition()
		{
			...
			Quaternion phi180 = OriginalRotation * Quaternion.CreateFromAxisAngle(Vector3.Up, MathF.PI);
			...
			Waiters.Interpolate(0.75, delegate(float s)
			{
				if (base.Enabled)
				{
					...
					Vector3 vector11 = (antiCubes.Scale = (maps.Scale = vector8));
					...
					vector11 = (goldenCubes2.Position = (antiCubes2.Position = vector8));
					...
					Quaternion quaternion11 = (antiCubes3.Rotation = (maps3.Rotation = quaternion8));
		...
		private void StartOutTransition()
		{
			...
			IWaiter waiter = Waiters.Interpolate(0.5, delegate(float s)
			{
		...
		protected override void Dispose(bool disposing)
		{
			if (oldTextureCache != null)
			{
				...
				ArtObjectMaterializer artObjectMaterializer = new ArtObjectMaterializer(AoInstance.ArtObject);
				artObjectMaterializer.RecomputeTexCoords(widen: true);
			}
		...
		public override void Update(GameTime gameTime)
		{
			...
			if (InputManager.RotateRight == FezButtonState.Pressed)
			{
				if (TomeOpen)
				{
					if (TomePageIndex > 0)
					{
						...
						Waiters.Interpolate(0.25, delegate(float s)
						{
							...
							TomePages.Groups[tpi2].Rotation = Quaternion.CreateFromAxisAngle(Vector3.UnitY, MathF.PI * -3f / 4f * s * (1f - (float)tpi2 / 8f * 0.05f));
						}, delegate
						{
							TomePages.Groups[tpi2].Rotation = Quaternion.Identity;
							if (tpi2 != 8)
							{
								TomePages.Groups[tpi2 + 1].Enabled = false;
							}
						});
					}
				}
				else
				{
					...
					{
						OriginalRotation *= Quaternion.CreateFromAxisAngle(Vector3.Up, -MathF.PI / 2f);
					}
				}
			}
				...
				if (TomeOpen)
				{
					if (TomePageIndex <= 8)
					{
						...
						Waiters.Interpolate(0.25, delegate(float s)
						{
							...
							if (tpi != 8)
							{
								TomePages.Groups[tpi + 1].Enabled = true;
							}
							TomePages.Groups[tpi].Rotation = Quaternion.CreateFromAxisAngle(Vector3.UnitY, MathF.PI * -3f / 4f * s * (1f - (float)tpi / 8f * 0.05f));
						}, delegate
						{
							TomePages.Groups[tpi].Rotation = Quaternion.CreateFromAxisAngle(Vector3.UnitY, MathF.PI * -3f / 4f * (1f - (float)tpi / 8f * 0.05f));
						});
					}
				}
				else
			...
			Quaternion quaternion12 = (antiCubes.Rotation = (maps.Rotation = quaternion9));
			Mesh antiCubes2 = AntiCubes;
			Mesh maps2 = Maps;
			Mesh hidingPlanes2 = HidingPlanes;
			Vector3 vector = (GoldenCubes.Position = AoInstance.Position);
			Vector3 vector3 = (hidingPlanes2.Position = vector);
			Vector3 vector6 = (antiCubes2.Position = (maps2.Position = vector3));
			TransformArtifacts();
			HandleSelection();
			TestForTempleOfLove();
		}
		...
		private void MoveAndRotate(MenuCubeFace cf, Vector2 diff)
		{
			...
			Waiters.Interpolate(0.15, delegate(float s)
			{
				for (int j = 0; j < 4; j++)
				{
					Group group2 = Highlights.Groups[(int)cf * 4 + j];
					...
					group2.Position = (vector.X + diff.X * s) * (float)cf.GetSpacing() / 16f * cf.GetRight() + (vector.Y + diff.Y * s) * (float)cf.GetSpacing() / 16f * -Vector3.UnitY + scale / 2f * (cf.GetForward() + Vector3.Up - cf.GetRight()) + cf.GetForward() * (-0.4375f + (float)cf.GetSize() / 2f / 16f * (float)Math.Sin(s * MathF.PI)) + (Vector3.Down + cf.GetRight()) * cf.GetOffset() / 16f;
					group2.Rotation = Quaternion.CreateFromAxisAngle(axis, s * MathF.PI * (float)sgn);
				}
			}, delegate
			{
				for (int i = 0; i < 4; i++)
		...
		private void TransformArtifacts()
		{
			...
			foreach (ArtObjectInstance artifactAO in ArtifactAOs)
			{
				...
				artifactAO.Rotation = Quaternion.CreateFromAxisAngle(Vector3.Up, MathF.PI) * AoInstance.Rotation;
		...
		private void HandleSelection()
		{
			...
			if ((TomeZoom && InputManager.GrabThrow == FezButtonState.Pressed) || (TomeOpen && InputManager.CancelTalk == FezButtonState.Pressed))
			{
				...
				else
				{
					...
					for (int num2 = TomePageIndex - 1; num2 >= 0; num2--)
					{
						...
						{
							s = Easing.EaseOut(FezMath.Saturate(1f - s), EasingType.Quadratic);
							TomePages.Groups[i1].Rotation = Quaternion.CreateFromAxisAngle(Vector3.UnitY, MathF.PI * -3f / 4f * s * (1f - (float)i1 / 8f * 0.05f));
						}, delegate
						{
			...
			else
			{
				...
				if (zoomed)
				{
					...
					BlendingMode? blendingMode3 = (AntiCubes.Blending = (GoldenCubes.Blending = BlendingMode.Alphablending));
				}
				else
				{
					BlendingMode? blendingMode3 = (AntiCubes.Blending = (GoldenCubes.Blending = BlendingMode.Opaque));
					CameraManager.OriginalDirection = OriginalDirection;
					GameState.ShowScroll(Face.GetTitle(), 0f, onTop: true);
					TomeOpen = false;
				}
				...
				case MenuCubeFace.Artifacts:
				{
					...
					for (int k = 0; k <= oid; k++)
					{
						if (k != oid && ArtifactAOs[k].ArtObjectName == "TOME_BAO")
						{
							oid++;
						}
					}
				...
				case MenuCubeFace.Maps:
				{
					...
					Waiters.Interpolate(0.25, delegate(float s)
					{
						...
						for (int m = 0; m < 6; m++)
						{
							Group group = Maps.Groups[oid * 6 + m];
							AoInstance.Material.Opacity = FezMath.Saturate(1f - s * 1.5f);
							AoInstance.MarkDirty();
							group.Position = Vector3.Lerp(originalMapPositions[m], originalMapPositions[m] + zoomedFace.GetForward() * 12f + middleOffset, s);
							zero += group.Position;
						}
						zero /= 6f;
		...
		private void DoTomeOpen(ArtObjectInstance ao, float s)
		{
			...
			(Matrix.CreateFromQuaternion(Quaternion.CreateFromAxisAngle(Vector3.Up, MathF.PI) * AoInstance.Rotation) * Matrix.CreateTranslation(vector3) * Matrix.CreateFromAxisAngle(Vector3.UnitY, MathF.PI * -3f / 4f * s) * Matrix.CreateTranslation(AoInstance.Position + vector * 12f - vector3)).Decompose(out var _, out var rotation, out var translation);
		...
		private void UpdateControllerTexture(object sender, EventArgs e)
		{
			if (GamepadState.Layout == GamepadState.GamepadLayout.Xbox360)
			{
				...
				{
					group.Texture = mystery2Xbox;
				}
			}
			else
			{
				Group[] array2 = mystery2Groups;
				foreach (Group group2 in array2)
				{
					group2.Texture = mystery2Sony;
				}
			}
		}
	}
