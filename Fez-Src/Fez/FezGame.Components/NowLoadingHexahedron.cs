...
namespace FezGame.Components
{
	internal class NowLoadingHexahedron : DrawableGameComponent
	{
		...
		private static Phases Phase = Phases.VectorOutline;
		...
		public override void Initialize()
		{
			...
			WireCube.Rotation = Quaternion.CreateFromAxisAngle(Vector3.Right, (float)Math.Asin(Math.Sqrt(2.0) / Math.Sqrt(3.0))) * Quaternion.CreateFromAxisAngle(Vector3.Up, MathF.PI / 4f);
			...
			SolidCube.Rotation = Quaternion.CreateFromAxisAngle(Vector3.Right, (float)Math.Asin(Math.Sqrt(2.0) / Math.Sqrt(3.0))) * Quaternion.CreateFromAxisAngle(Vector3.Up, MathF.PI / 4f);
		...
		public override void Update(GameTime gameTime)
		{
			...
			Vector3 vector8 = (rays.Position = (flare.Position = vector5));
			Quaternion quaternion2 = (Flare.Rotation = (Outline.Rotation = CameraManager.Rotation));
			quaternion2 = (SolidCube.Rotation = (WireCube.Rotation = CameraManager.Rotation * Quaternion.CreateFromAxisAngle(Vector3.Up, Phi)));
			...
			{
				float num12 = FezMath.Saturate(num / 7.5f);
				if (num12 == 1f)
				{
					Phase = Phases.Rays;
					SincePhaseStarted = TimeSpan.Zero;
