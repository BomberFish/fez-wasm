	...
	internal class OrreryHost : GameComponent
	{
		...
		public override void Update(GameTime gameTime)
		{
			if (!GameState.Loading && !GameState.Paused && !GameState.InMap && !GameState.InMenuCube && !GameState.InFpsMode)
			{
				...
				(Matrix.CreateTranslation(1.875f, 0f, -1.875f) * Matrix.CreateFromQuaternion(Earth.Rotation * Quaternion.CreateFromAxisAngle(Vector3.UnitY, -MathF.PI / 2f)) * Matrix.CreateTranslation(Sun.Position * FezMath.XZMask + Vector3.UnitY * EarthBranchHeight)).Decompose(out var scale, out var rotation, out var translation);
				...
				(Matrix.CreateTranslation(3.875f, 0f, -3.875f) * Matrix.CreateFromQuaternion(PlanetW.Rotation * Quaternion.CreateFromAxisAngle(Vector3.UnitY, MathF.PI)) * Matrix.CreateTranslation(Sun.Position * FezMath.XZMask + Vector3.UnitY * PlanetWBranchHeight)).Decompose(out scale, out rotation, out translation);
