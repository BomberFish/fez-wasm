	...
	public class PickupsHost : DrawableGameComponent
	{
		...
		private void TryFloat(PickupState pickup, float elapsedSeconds)
		{
			...
			if (pickup.TouchesWater && !physicsState.Floating)
			{
				...
				if (type.IsBuoyant())
				{
					float num3 = LevelManager.WaterHeight - 0.8125f + pickup.FloatMalus;
					float num4 = num3 - instance.Position.Y;
					if (sinceLevelChanged <= 1f && LevelManager.WaterType == LiquidType.Sewer && !physicsState.IgnoreClampToWater && Math.Abs(num4) > 0.5f)
					{
					...
					{
						physicsState.Velocity += num4 * 0.00875f * Vector3.UnitY;
					}
				}
			}
			...
			{
				float num5 = LevelManager.WaterHeight - 0.8125f + pickup.FloatMalus;
				float num6 = num5 - instance.Position.Y;
				if (Math.Abs(num6) > 0.5f)
				{
					physicsState.NoVelocityClamping = true;
					physicsState.Velocity += num6 * Vector3.UnitY;
				}
			}
		}
		...
		private void SpawnDust(TrileInstance instance, float opacity, AnimatedTexture animation, bool onRight, bool onLeft)
		{
			...
			if (onRight)
			{
				...
				{
					OriginalRotation = Quaternion.CreateFromAxisAngle(Vector3.UnitX, (float)b.AsNumeric() * MathF.PI),
					Doublesided = true,
					Loop = false,
					Opacity = opacity,
					Timing = 
			...
			if (onLeft)
			{
				...
				{
					OriginalRotation = Quaternion.CreateFromAxisAngle(Vector3.Up, MathF.PI) * Quaternion.CreateFromAxisAngle(Vector3.UnitX, (float)b.AsNumeric() * MathF.PI),
					Doublesided = true,
					Loop = false,
					Opacity = opacity,
					Timing = 
