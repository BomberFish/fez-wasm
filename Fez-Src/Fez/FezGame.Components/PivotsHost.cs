		...
		private class PivotState
		{
			...
			private void ForceSpinTo(int initialSpins)
			{
				...
				for (int i = 0; i < num; i++)
				{
					...
					float num2 = MathF.PI / 2f * (float)Math.Sign(initialSpins);
				...
				HandleAo.Rotation *= Quaternion.CreateFromAxisAngle(Vector3.UnitY, MathF.PI / 2f * (float)initialSpins);
			}
			...
			public bool Update(TimeSpan elapsed)
			{
				...
				case SpinAction.Spinning:
				{
					...
					float num2 = (float)((num < 0.800000011920929) ? (num / 0.800000011920929) : (1.0 + Math.Sin((num - 0.800000011920929) / 0.20000000298023224 * 6.2831854820251465 * 2.0) * 0.009999999776482582 * (1.0 - num) / 0.20000000298023224));
					num2 = Easing.EaseIn(num2, EasingType.Quartic);
					float num3 = num2 * (MathF.PI / 2f) * (float)SpinSign;
					...
					for (int i = 0; i < OriginalStates.Length; i++)
					{
						...
						trileInstance.Position = Vector3.Transform(vector.XYZ() + new Vector3(0.5f) - position, quaternion) + position - new Vector3(0.5f);
						trileInstance.Phi = FezMath.WrapAngle(vector.W + num3);
						LevelMaterializer.GetTrileMaterializer(trileInstance.Trile).UpdateInstance(trileInstance);
					}
