...
namespace FezGame.Components
{
	public class PlayerCameraControl : GameComponent
	{
			...
			if ((!PlayerManager.Action.PreventsRotation() || GameState.InMap || GameState.InMenuCube || GameState.InFpsMode) && PlayerManager.CanRotate && (!LevelManager.Flat || PlayerManager.Action == ActionType.GrabTombstone || GameState.InMap || GameState.InFpsMode || GameState.InMenuCube) && !GameState.Paused && !GameState.InCutscene)
			{
				...
				if (CameraManager.Viewpoint == Viewpoint.Perspective || CameraManager.RequestedViewpoint == Viewpoint.Perspective || GameState.InMap)
				{
					if (CameraManager.Viewpoint == Viewpoint.Perspective && !GameState.InMenuCube && !GameState.InMap)
					{
						...
						Vector3 vector2 = (StickyCenter = (CameraManager.Center = Vector3.Lerp(CameraManager.Center, PlayerManager.Position + num * Vector3.UnitY, 0.075f)));
					}
					...
					{
						Vector3 vector3 = Vector3.Transform(CameraManager.OriginalDirection, Matrix.CreateFromAxisAngle(Vector3.Up, MathF.PI / 2f));
						Vector3 to = Vector3.Transform(CameraManager.OriginalDirection, Matrix.CreateFromAxisAngle(vector3, -MathF.PI / 2f));
						Vector2 vector4 = InputManager.FreeLook / (GameState.MenuCubeIsZoomed ? 1.75f : 6.875f);
						float step = 0.1f;
						if (GameState.InMap && MouseState.LeftButton.State == MouseButtonStates.Dragging)
						{
							vector4 = Vector2.Clamp(new Vector2(-MouseState.LeftButton.DragState.Movement.X, MouseState.LeftButton.DragState.Movement.Y) / (300f * GraphicsDeviceService.GraphicsDevice.GetViewScale()), -Vector2.One, Vector2.One) / 3.4375f;
							step = 0.2f;
							lastFactors = vector4;
						}
						...
						if (GameState.InMap)
						{
							vector4 *= new Vector2(3.425f, 1.725f);
							vector4.Y += 0.25f;
							vector4.X += 0.5f;
						}
						Vector3 to2 = FezMath.Slerp(FezMath.Slerp(CameraManager.OriginalDirection, vector3, vector4.X), to, vector4.Y);
						if (!CameraManager.ActionRunning)
						{
			...
			if (!GameState.Paused && (!CameraManager.Constrained || CameraManager.PanningConstraints.HasValue || GameState.InMap) && !PlayerManager.Hidden)
			{
				if ((CameraManager.ActionRunning || CameraManager.ForceTransition) && CameraManager.Viewpoint != Viewpoint.Perspective)
				{
					if (InputManager.FreeLook != Vector2.Zero)
					{
						...
						Vector2 vector5 = InputManager.FreeLook;
						if (MouseState.LeftButton.State == MouseButtonStates.Dragging)
						{
							vector5 = -vector5;
						}
						...
						Vector2 vector6 = new Vector2(CameraManager.Radius, CameraManager.Radius / CameraManager.AspectRatio) * 0.4f / viewScale;
						StickyCenter = Vector3.Lerp(StickyCenter, PlayerManager.Position + (vector5.X * CameraManager.Viewpoint.RightVector() * vector6.X + vector5.Y * Vector3.UnitY * vector6.Y), 0.05f);
					}
		...
		private void RotateViewLeft()
		{
			...
			if (CameraManager.Viewpoint == Viewpoint.Perspective || GameState.InMap)
			{
				CameraManager.OriginalDirection = Vector3.Transform(CameraManager.OriginalDirection, Quaternion.CreateFromAxisAngle(Vector3.Up, -MathF.PI / 2f));
		...
		{
			int distance = CameraManager.Viewpoint.GetDistance(view);
