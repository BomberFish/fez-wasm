	...
	internal class PushSwitchesHost : GameComponent
	{
		private class SwitchState
		{
			...
			public void Update(TimeSpan elapsed)
			{
				...
				{
					float num9 = sinceActionStarted.Ticks;
					TimeSpan comeBackDuration = HalfPushDuration;
					num2 = num9 / (float)comeBackDuration.Ticks * 0.1875f;
					if (!flag)
					{
						ReleaseSound.EmitAt(Group.Triles.First().Center);
						switchAction = SwitchAction.ComingBack;
					}
					long ticks = sinceActionStarted.Ticks;
					comeBackDuration = HalfPushDuration;
					if (ticks >= comeBackDuration.Ticks)
					{
				...
				{
					float num10 = sinceActionStarted.Ticks;
					TimeSpan comeBackDuration = FullPushDuration;
					float num11 = num10 / (float)comeBackDuration.Ticks;
					num2 = 0.1875f + Easing.EaseIn(num11, EasingType.Quadratic) * 0.625f;
					if (!flag && Permanence == SwitchPermanence.Volatile)
					{
						switchAction = SwitchAction.ComingBack;
					}
					long ticks2 = sinceActionStarted.Ticks;
					comeBackDuration = FullPushDuration;
					if (ticks2 >= comeBackDuration.Ticks)
					{
				...
				{
					float num6 = lastStep;
					float num7 = sinceActionStarted.Ticks;
					TimeSpan comeBackDuration = ComeBackDuration;
					num2 = num6 - num7 / ((float)comeBackDuration.Ticks * lastStep) * lastStep;
					float num8 = sinceActionStarted.Ticks;
					comeBackDuration = ComeBackDuration;
					if (num8 >= (float)comeBackDuration.Ticks * lastStep)
					{
				...
				{
					float num3 = lastStep;
					float num4 = sinceActionStarted.Ticks;
					TimeSpan comeBackDuration = ComeBackDuration;
					num2 = num3 - num4 / ((float)comeBackDuration.Ticks * lastStep) * lastStep;
					float num5 = sinceActionStarted.Ticks;
					comeBackDuration = ComeBackDuration;
					if (num5 >= (float)comeBackDuration.Ticks * lastStep)
					{
						switchAction = SwitchAction.Up;
					}
					break;
				...
				float num12 = Group.Triles[0].Position.Y - y;
				foreach (TrileInstance trile3 in Group.Triles)
				{
					trile3.PhysicsState.Velocity = new Vector3(0f, num12, 0f);
					if (num12 != 0f)
					{
