...
namespace FezGame.Components
{
	internal class PyramidHost : DrawableGameComponent
	{
		...
		public override void Update(GameTime gameTime)
		{
			...
			if (Math.Abs(vector2.X - DoorCenter.X) < 1f && Math.Abs(vector2.Y - (DoorCenter.Y + vector.Y)) < 1f)
			{
				Vector3 a = Vector3.Transform(-Vector3.UnitZ, MotherCubeAo.Rotation);
				if (FezMath.AngleBetween(a, CameraManager.Viewpoint.ForwardVector()) < 0.25f)
				{
					DoCapture = true;
					TimeAccumulator = 0f;
		...
		private void UpdateRays(float elapsedSeconds)
		{
			...
			Vector3 vector2 = (FlareMesh.Position = (RaysMesh.Position = PlayerManager.Center));
