	...
	internal class RotatingGroupsHost : GameComponent, IRotatingGroupService, IScriptingBase
	{
		private class RotatingGroupState
		{
			...
			public void Update(float elapsedSeconds)
			{
				...
				{
					float num = FezMath.Saturate(SinceChanged / (0.75f * (float)Turns));
					float num2 = Easing.EaseInOut(FezMath.Saturate(num / 0.75f), EasingType.Quartic, EasingType.Quadratic);
					float num3 = num2 * (MathF.PI / 2f) * (float)SpinSign * (float)Turns;
					Matrix matrix = Matrix.CreateFromAxisAngle(Vector3.UnitY, num3);
					Quaternion quaternion = Quaternion.CreateFromAxisAngle(Vector3.UnitY, num3);
					if (!PlayerManager.IsOnRotato && num2 < 0.5f)
					{
					...
					if ((GroundedOn || HeldOnto) && num2 > 0.1f && !CameraManager.ForceTransition)
					{
					...
					for (int i = 0; i < OriginalStates.Length; i++)
					{
						...
						trileInstance.SetPhiLight(vector2.W + num3);
			...
			public void Rotate(bool clockwise, int turns)
			{
				...
				if (sSpin != null)
				{
					SoundEmitter soundEmitter = sSpin.EmitAt(Center, loop: false, RandomHelper.Centered(0.10000000149011612), paused: false);
					soundEmitter.FadeDistance = 50f;
				}
			}
			...
			{
				float num = MathF.PI / 2f * (float)SpinSign * (float)Turns;
