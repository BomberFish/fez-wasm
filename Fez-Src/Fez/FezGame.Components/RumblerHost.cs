...
namespace FezGame.Components
{
	internal class RumblerHost : GameComponent
	{
		...
		private TimeSpan CurrentDuration => (CurrentSignal == VibrationMotor.None) ? SilenceDuration : SignalDuration;
			...
			if (ArtObject != null)
			{
				...
				if (GameState.SaveData.ThisLevel.InactiveArtObjects.Contains(ArtObject.Id))
				{
					if (ArtObject.ActorSettings.AttachedGroup.HasValue)
					{
						...
						TrileGroup trileGroup = LevelManager.Groups[ArtObject.ActorSettings.AttachedGroup.Value];
