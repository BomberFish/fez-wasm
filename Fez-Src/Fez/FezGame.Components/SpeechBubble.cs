...
namespace FezGame.Components
{
	public class SpeechBubble : DrawableGameComponent, ISpeechBubbleManager
	{
		...
		public bool Hidden => !show && !changingText;
		...
		public void ChangeText(string toText)
		{
			...
			changingText = true;
			IWaiter waiter = Waiters.Wait(() => sinceShown == 0f, delegate
			{
				if (changingText)
				{
					changingText = false;
		...
		{
			BaseEffect baseEffect3 = (textMesh.Effect = (canvasMesh.Effect = new DefaultEffect.Textured()));
			...
			Texture texture3 = (scalableMiddle.Texture = (scalableTop.Texture = scalableBottom.Texture));
		...
		private void OnTextChanged(bool update)
		{
			...
			if (Font != SpeechFont.Zuish)
			{
				...
				string text2 = GTR.FillInGlyphs(textString, out glyphLocations);
				if (Culture.IsCJK)
				{
					num2 /= 2f;
				}
				StringBuilder stringBuilder = new StringBuilder(WordWrap.Split(text2, spriteFont, val / num2));
				if (Culture.IsCJK)
				{
					num2 *= 2f;
				}
			...
			scalableMiddleSize = vector + Vector2.One * 4f * 2f * num7 + Vector2.UnitX * 4f * 2f * num7;
			if (Font == SpeechFont.Zuish)
			{
				scalableMiddleSize += Vector2.UnitY * 2f;
			}
			int num8 = (int)scalableMiddleSize.X;
			int num9 = (int)scalableMiddleSize.Y;
			if (Culture.IsCJK && Font == SpeechFont.Pixel)
			{
				num2 *= 2f;
				num8 *= 2;
			...
			this.text = new RenderTarget2D(base.GraphicsDevice, num8, num9, mipMap: false, base.GraphicsDevice.PresentationParameters.BackBufferFormat, base.GraphicsDevice.PresentationParameters.DepthStencilFormat, 0, RenderTargetUsage.PreserveContents);
		...
		public override void Draw(GameTime gameTime)
		{
			...
			if (Font == SpeechFont.Zuish)
			{
				textMesh.Rotation *= Quaternion.CreateFromYawPitchRoll(0f, 0f, -MathF.PI / 2f);
			}
