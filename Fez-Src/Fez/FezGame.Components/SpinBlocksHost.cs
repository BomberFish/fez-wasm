	...
	internal class SpinBlocksHost : GameComponent
	{
		private class SpinBlockState
		{
			...
			public void Update(TimeSpan elapsed)
			{
				...
				{
					double num5 = FezMath.Saturate(SinceChanged.TotalSeconds / 0.10000000149011612);
					float num6 = (float)Math.Sin(num5 * 0.7853981852531433);
					Quaternion quaternion2 = Quaternion.CreateFromAxisAngle(ArtObject.ActorSettings.SpinView.ForwardVector(), -MathF.PI / 2f * num6 * 0.1f);
				...
				case SpinState.Spinning:
				{
					...
					Quaternion quaternion = Quaternion.CreateFromAxisAngle(ArtObject.ActorSettings.SpinView.ForwardVector(), MathF.PI / 2f * num2 * 1.1f - MathF.PI / 20f);
			...
			private void Rotate()
			{
				...
				Quaternion rotation = Quaternion.CreateFromAxisAngle(ArtObject.ActorSettings.SpinView.ForwardVector(), MathF.PI / 2f);
				...
				TrileInstance[] array3 = array;
				foreach (TrileInstance instance in array3)
				{
