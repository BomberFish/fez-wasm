	...
	internal class SplitUpCubeHost : DrawableGameComponent
	{
		...
		private class SwooshingCube
		{
			...
			public SwooshingCube(TrileInstance instance, Mesh destinationMesh, Vector3 Offset, Quaternion Rotation)
			{
				...
				Group group = Cube.AddGroup();
				group.Geometry = geometry2;
				Group group2 = Trail.AddGroup();
				group2.Geometry = (TrailGeometry = new IndexedUserPrimitives<FezVertexPositionColor>(TrailVertices = new FezVertexPositionColor[0], TrailIndices = new int[0], PrimitiveType.TriangleList));
				Instance = instance;
				Vector3 vector = (lastPoint = instance.Center);
			...
			private void AlignLastSegment()
			{
				...
				Matrix matrix = new Matrix(vector3.X, vector2.X, vector.X, 0f, vector3.Y, vector2.Y, vector.Y, 0f, vector3.Z, vector2.Z, vector.Z, 0f, 0f, 0f, 0f, 1f);
				Quaternion quaternion = Quaternion.Inverse(Quaternion.CreateFromRotationMatrix(matrix));
				int num = TrailVertices.Length - 6;
				for (int i = 0; i < 6; i++)
				{
					float num2 = (float)i / 6f * (MathF.PI * 2f);
			...
			public void Update(GameTime gameTime)
			{
				for (int i = 0; i < Spline.Points.Length; i++)
				{
					...
					zero += sideDirection * 3.5f * (0.7f - (float)Math.Sin(num * (MathF.PI * 2f) + MathF.PI / 4f));
					zero += Vector3.Up * 2f * (0.7f - (float)Math.Cos(num * (MathF.PI * 2f) + MathF.PI / 4f));
		...
		private void TryInitialize()
		{
			if (WireframeCube == null)
			{
				...
				for (int i = 0; i < 7; i++)
				{
					...
					Vector3[] cubeOffsets = CubeOffsets;
			...
			Vector3[] cubeOffsets2 = CubeOffsets;
			foreach (Vector3 position in cubeOffsets2)
			{
			...
			Quaternion quaternion3 = (SolidCubes.Rotation = (WireframeCube.Rotation = Quaternion.CreateFromAxisAngle(Vector3.Right, (float)Math.Asin(Math.Sqrt(2.0) / Math.Sqrt(3.0))) * Quaternion.CreateFromAxisAngle(Vector3.Up, MathF.PI / 4f)));
		...
		public override void Update(GameTime gameTime)
		{
			...
			{
				SwooshingCube swooshingCube = TrackedCollects[num2];
		...
		private void TryAssembleCube()
		{
			...
			Waiters.Wait(() => !GameState.Loading && PlayerManager.Action.AllowsLookingDirectionChange() && SpeechBubble.Hidden && !GameState.ForceTimePaused && PlayerManager.CanControl && !PlayerManager.Action.DisallowsRespawn() && CameraManager.ViewTransitionReached && !PlayerManager.InDoorTransition && PlayerManager.CarriedInstance == null, delegate
			{
				Waiters.Wait(0.0, delegate
				{
					...
					Vector3 vector3 = PlayerManager.Position + Vector3.UnitY * ((float)Math.Sin(timeAcc.TotalSeconds * 3.1415927410125732) * 0.1f + 2f) - FezMath.HalfVector;
					vector3 = vector3 * (Vector3.One - vector) - vector2 * (LevelManager.Size / 2f - vector * 2f) + vector * LevelManager.Size / 2f;
					TrileInstance trileInstance = new TrileInstance(vector3, LevelManager.TrileSet.Triles.Values.Last((Trile x) => x.ActorSettings.Type == ActorType.CubeShard).Id);
		...
		public override void Draw(GameTime gameTime)
		{
			...
			if (SolidCubesVisible || WirecubeVisible)
			{
				...
				Vector3 vector3 = (SolidCubes.Position = (WireframeCube.Position = position + Vector3.UnitY * ((float)Math.Sin(timeAcc.TotalSeconds * 3.1415927410125732) * 0.1f + 2f)));
				Quaternion quaternion3 = (SolidCubes.Rotation = (WireframeCube.Rotation = Quaternion.CreateFromAxisAngle(Vector3.Up, (float)gameTime.ElapsedGameTime.TotalSeconds) * WireframeCube.Rotation));
