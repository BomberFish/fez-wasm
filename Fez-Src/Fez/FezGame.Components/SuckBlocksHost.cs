...
namespace FezGame.Components
{
	internal class SuckBlocksHost : GameComponent
	{
				...
				foreach (Volume hostingVolume in HostingVolumes)
				{
					...
					if (num < 0.01f)
					{
						...
						if (trackedSuckBlock.Action == SuckBlockAction.Processing)
						{
							...
							float num2 = trackedSuckBlock.SinceActionChanged.Ticks;
							TimeSpan processingTime = SuckBlockState.ProcessingTime;
							instance.Position = Vector3.Lerp(value2, vector9, num2 / (float)processingTime.Ticks);
						...
						if (trackedSuckBlock.Action == SuckBlockAction.Rejected && instance.PhysicsState.Velocity.XZ() == Vector2.Zero)
						{
							...
							Vector3 vector10 = new Vector3(num3, 0.75f, num3) * vector3;
		...
		private static void DisableTrile(TrileInstance instance)
		{
			...
			CollisionType collisionType7 = (faces[FaceOrientation.Left] = (faces2[FaceOrientation.Right] = collisionType4));
		}
