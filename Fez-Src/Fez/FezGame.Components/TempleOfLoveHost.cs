...
namespace FezGame.Components
{
	internal class TempleOfLoveHost : DrawableGameComponent
	{
		...
		private Phases Phase = Phases.None;
		...
		private void TryInitialize()
		{
			...
			Group group = WireHeart.AddGroup();
			group.Geometry = new IndexedUserPrimitives<FezVertexPositionColor>(new FezVertexPositionColor[12]
			{
			...
			int num = 0;
			Vector3[] pieceOffsets2 = PieceOffsets;
			foreach (Vector3 position in pieceOffsets2)
			{
				Trile[] array3 = array2;
				foreach (Trile trile in array3)
				{
					Group group2 = CrumblingHeart.AddGroup();
					group2.Geometry = new IndexedUserPrimitives<VertexPositionNormalTextureInstance>(trile.Geometry.Vertices.ToArray(), trile.Geometry.Indices, trile.Geometry.PrimitiveType);
					group2.Position = position;
					group2.Enabled = GameState.SaveData.PiecesOfHeart > num;
					group2.Rotation = Quaternion.CreateFromAxisAngle(Vector3.UnitZ, MathF.PI / 2f);
				}
				num++;
			}
			Quaternion quaternion3 = (WireHeart.Rotation = (CrumblingHeart.Rotation = Quaternion.CreateFromAxisAngle(Vector3.UnitZ, -MathF.PI / 4f)));
			WireHeart.BakeTransform<FezVertexPositionColor>();
			CrumblingHeart.BakeTransform<VertexPositionNormalTextureInstance>();
			foreach (Group group3 in CrumblingHeart.Groups)
			{
				IndexedUserPrimitives<VertexPositionNormalTextureInstance> indexedUserPrimitives = group3.Geometry as IndexedUserPrimitives<VertexPositionNormalTextureInstance>;
				...
				group3.CustomData = zero / indexedUserPrimitives.Vertices.Length;
			}
			RaysMesh = new Mesh
			{
			...
			Vector3 vector2 = (WireHeart.Position = (CrumblingHeart.Position = HeartCenter));
		}
		...
		public override void Update(GameTime gameTime)
		{
			...
			Vector3 vector3 = (WireHeart.Position = (CrumblingHeart.Position = HeartCenter + Vector3.UnitY * (float)Math.Sin(TimeAccumulator / 2f) / 2f));
			...
			case Phases.CrumbleOut:
			{
				...
				{
					Vector3 vector5 = (Vector3)group.CustomData;
					float num3 = 1f - (vector5.Y * 2f + vector5.Z + vector5.X);
					float num4 = (float)Math.Pow(Math.Max(PhaseTime - num3, 0f) * 0.875f, 2.0);
					group.Position = new Vector3(0f, num4, 0f) + vector5 * num4 / 5f;
					group.Rotation = Quaternion.CreateFromAxisAngle(Vector3.Normalize(vector5), num4 / 10f);
				}
			...
			case Phases.ShineReboot:
				...
				{
					Vector3 vector4 = (Vector3)group2.CustomData;
					float num = 1f - (vector4.Y * 2f + vector4.Z + vector4.X);
					float num2 = (float)Math.Pow(Math.Max(PhaseTime + 7f - num, 0f) * 0.875f, 2.0);
					group2.Position = new Vector3(0f, num2, 0f) + vector4 * num2 / 5f;
					group2.Rotation = Quaternion.CreateFromAxisAngle(Vector3.Normalize(vector4), num2 / 10f);
				}
		...
		{
			bool flag = PhaseTime > 1.5f;
			MakeRay();
			if (flag)
			{
				MakeRay();
			}
			for (int num = RaysMesh.Groups.Count - 1; num >= 0; num--)
			{
				Group group = RaysMesh.Groups[num];
				...
				if (rayState.Age > 1f)
				{
					RaysMesh.RemoveGroupAt(num);
				}
			}
			RaysMesh.AlwaysOnTop = false;
			Vector3 vector2 = (FlareMesh.Position = (RaysMesh.Position = HeartCenter));
			Quaternion quaternion2 = (FlareMesh.Rotation = (RaysMesh.Rotation = CameraManager.Rotation));
