	...
	internal class TetrisPuzzleHost : GameComponent
	{
		...
		public override void Update(GameTime gameTime)
		{
			...
			foreach (TrileInstance instance in Blocks)
			{
				...
				Vector3 vector3 = PuzzleCenter + value3 * 1.75f;
				Vector3 vector4 = (vector3 - instance.Center) * FezMath.XZMask;
				float num5 = Math.Max(vector4.Length(), 0.1f);
				instance.PhysicsState.Velocity += 0.25f * (vector4 / num5) * (float)gameTime.ElapsedGameTime.TotalSeconds;
				if (num5 <= 0.1f && value3.Y == 0f && instance != PlayerManager.Ground.First && !Blocks.Any((TrileInstance x) => x.PhysicsState.Ground.First == instance) && Blocks.Any((TrileInstance b) => b != instance && FezMath.AlmostEqual(b.Position.Y, instance.Position.Y) && Math.Abs((b.Center - instance.Center).Dot(depthMask)) < 1.75f))
				{
					int num6 = Math.Sign((instance.Center - PuzzleCenter).Dot(depthMask));
					if (num6 == 0)
					{
						num6 = 1;
					}
					for (int i = -2; i <= 2; i++)
					{
						int num7 = SpiralTraversal[i + 2] * num6;
						Vector3 tetativePosition = vector * instance.PhysicsState.Center + PuzzleCenter * depthMask + depthMask * num7 * 1.75f;
				...
				if (RightPositions >= 4 || !(num5 <= 0.1f) || !instance.PhysicsState.Grounded)
				{
					continue;
				}
			...
			if (RightPositions < 4 && (num3 == 2 || num4 == 2))
			{
				RightPositions += 2;
			}
		}
