	...
	public class TombstonesHost : DrawableGameComponent
	{
		private class TombstoneState
		{
			...
			public TombstoneState(TombstonesHost host, ArtObjectInstance ao)
			{
				...
				if (GameState.SaveData.ThisLevel.PivotRotations.TryGetValue(ArtObject.Id, out var value) && value != 0)
				{
					...
					for (int i = 0; i < num; i++)
					{
						...
						float angle = MathF.PI / 2f * (float)Math.Sign(value);
			...
			public bool Update(TimeSpan elapsed)
			{
				...
				case SpinAction.Idle:
				{
					...
					bool flag2 = FezMath.AlmostEqual(Vector3.Transform(Vector3.UnitZ, ArtObject.Rotation).Abs(), CameraManager.Viewpoint.DepthMask());
					if (flag2 && flag && PlayerManager.CarriedInstance == null && PlayerManager.Grounded && PlayerManager.Action != ActionType.GrabTombstone && InputManager.FpsToggle != FezButtonState.Pressed && InputManager.GrabThrow == FezButtonState.Pressed && PlayerManager.Action != ActionType.ReadingSign)
					{
				...
				case SpinAction.Spinning:
				{
					...
					float num2 = (float)((num < 0.949999988079071) ? (num / 0.949999988079071) : (1.0 + Math.Sin((num - 0.949999988079071) / 0.05000000074505806 * 6.2831854820251465 * 2.0) * 0.009999999776482582 * (1.0 - num) / 0.05000000074505806));
					num2 = Easing.EaseIn(num2, EasingType.Linear);
					...
					{
						LastViewpoint = FezMath.OrientationFromDirection(Vector3.Transform(Vector3.Forward, ArtObject.Rotation).MaxClampXZ()).AsViewpoint();
						int num3 = Host.TrackedStones.Count((TombstoneState x) => x.LastViewpoint == LastViewpoint);
						TombstoneService.UpdateAlignCount(num3);
						if (num3 > 1)
						{
							TombstoneService.OnMoreThanOneAligned();
						}
						Host.StopSkullRotations = num3 == 4;
		...
		private void TryInitialize()
		{
			...
			if (base.Enabled)
			{
				...
				OriginalRotation = SkullAo.Rotation * Quaternion.CreateFromAxisAngle(Vector3.UnitY, MathF.PI / 2f);
			}
		}
		...
		private void RotateSkull()
		{
			...
			SkullAo.Rotation = InterpolatedRotation * Quaternion.CreateFromAxisAngle(Vector3.UnitY, -MathF.PI / 2f);
