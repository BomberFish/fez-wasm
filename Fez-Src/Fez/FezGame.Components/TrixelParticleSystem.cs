...
namespace FezGame.Components
{
	public class TrixelParticleSystem : DrawableGameComponent
	{
		private class Particle : ISimplePhysicsEntity, IPhysicsEntity
		{
			...
			public bool Grounded => Ground.NearLow != null || Ground.FarHigh != null;

			public bool Sliding => !FezMath.AlmostEqual(Velocity.X, 0f) || !FezMath.AlmostEqual(Velocity.Z, 0f);
		...
		private void SetupGeometry()
		{
			...
			group.Geometry = (Geometry = new ShaderInstancedIndexedPrimitives<VertexPositionNormalTextureInstance, Matrix>(PrimitiveType.TriangleList, 60));
		...
		public void AddImpulse(Vector3 energySource, float energy)
		{
			...
			foreach (Particle particle in particles)
			{
				if (particle.Enabled)
				{
					Vector3 vector3 = particle.Center * vector;
					Vector3 vector4 = vector3 - vector2;
					float num = (float)Math.Pow(1f / (1f + vector4.Length()), 1.5);
					if (num > 0.1f)
					{
						Vector3 vector5 = ((vector4 == Vector3.Zero) ? Vector3.Zero : Vector3.Normalize(vector4));
						particle.Velocity += vector5 * energy * 0.2f * num;
						particle.Static = false;
					}
				}
			}
