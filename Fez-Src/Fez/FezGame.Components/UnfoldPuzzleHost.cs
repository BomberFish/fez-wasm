				...
				foreach (TrileInstance block2 in Blocks)
				{
					if (!block2.PhysicsState.Background && block2.Enabled && block2.Position.Y >= 57f && (block2.Position.Z > 9f || block2.Center.X >= 11f || (block2.Center.X <= 6f && block2.Center.Y > 57f) || (block2.Center.X <= 5f && block2.Center.Y == 56.5f)))
					{
						...
						if (PlayerManager.HeldInstance != block2 && PlayerManager.PushedInstance != block2 && block2.PhysicsState.Grounded && block2.PhysicsState.Ground.First != PlayerManager.PushedInstance)
						{
							...
							Vector3 vector3 = PuzzleCenter + value * 1f;
							if (value.Y == 0f)
							{
								vector3 += Vector3.UnitZ;
							}
							Vector3 vector4 = (vector3 - block2.Center) * vector;
							float num = Math.Abs(vector4.X);
							float num2 = Math.Sign(vector4.X);
							...
							if (Math.Abs(vector4.X) <= 3f / 128f && Math.Abs(vector4.Y) <= 3f / 128f)
							{
								block2.PhysicsState.Velocity *= Vector3.UnitY;
								block2.PhysicsState.Center = block2.PhysicsState.Center * (Vector3.One - vector) + vector3 * vector;
					...
					Vector3 vector5 = (block2.Center * 16f).Round() / 16f;
					...
					{
						if ((PuzzleCenter + Slots1[i]) * vector == vector5 * vector)
						{
							RightPositions1++;
							break;
						}
					...
					{
						if ((PuzzleCenter + Slots2[j]) * vector == vector5 * vector)
						{
							RightPositions2++;
							break;
						}
					...
					{
						if ((PuzzleCenter + Slots3[k]) * vector == vector5 * vector)
						{
							RightPositions3++;
							break;
						}
					...
					{
						if ((PuzzleCenter + Slots4[l]) * vector == vector5 * vector)
						{
							RightPositions4++;
							break;
						}
