		...
		private class ValveState
		{
			...
			public ValveState(ValvesBoltsTimeswitchesHost host, ArtObjectInstance ao)
			{
				...
				if (!IsBolt && !IsTimeswitch && GameState.SaveData.ThisLevel.PivotRotations.TryGetValue(ArtObject.Id, out var value) && value != 0)
				{
					...
					for (int i = 0; i < num; i++)
					{
						float angle = MathF.PI / 2f * (float)num2;
			...
			public bool Update(TimeSpan elapsed)
			{
				...
				{
					Vector3 vector7 = (PlayerManager.Position - (ArtObject.Position - new Vector3(0f, 1f, 0f) + CenterOffset)) * vector;
					vector7.X += vector7.Z;
					Vector3 vector8 = vector7.Abs();
					bool flag = ((!IsBolt && !IsTimeswitch) ? (vector8.X < 1f && vector8.Y < 1f) : (vector8.X > 0.75f && vector8.X < 1.75f && vector8.Y < 1f));
					if (LevelManager.Flat)
					{
						flag = vector8.X < 1.5f && vector8.Y < 1f;
					}
					...
					{
						Vector3 vector9 = CameraManager.Viewpoint.ForwardVector();
						Vector3 vector10 = CameraManager.Viewpoint.DepthMask();
						Vector3 vector11 = (ArtObject.Position + CenterOffset) * vector10;
						PlayerManager.Position = PlayerManager.Position * vector + vector10 * vector11 - vector9 * 1.5f;
					...
					float num5 = ((ArtObject.ActorSettings.TimeswitchWindBackSpeed == 0f) ? 4f : ArtObject.ActorSettings.TimeswitchWindBackSpeed);
					float num6 = (float)elapsed.TotalSeconds / (num5 - 0.25f) * 2f;
					RewindSpeed = ((SinceChanged.TotalSeconds < 0.5) ? MathHelper.Lerp(0f, num6, (float)SinceChanged.TotalSeconds * 2f) : num6);
					float screwHeight = ScrewHeight;
					ScrewHeight = MathHelper.Clamp(ScrewHeight - RewindSpeed, 0f, 2f);
					float num7 = screwHeight - ScrewHeight;
					if (ScrewHeight == 0f && num7 != 0f)
					{
					...
					else if (num7 != 0f)
					{
						...
						eTimeswitchWindBack.VolumeFactor = FezMath.Saturate(num7 * 20f * ArtObject.ActorSettings.TimeswitchWindBackSpeed);
					}
					else
					{
					...
					TimeswitchScrewAo.Position -= Vector3.UnitY * num7;
					TimeswitchScrewAo.Rotation *= Quaternion.CreateFromAxisAngle(Vector3.UnitY, num7 * (MathF.PI / 2f) * -4f);
					break;
				}
				...
				case SpinAction.Spinning:
				{
					...
					float angle = num2 * (MathF.PI / 2f) * (float)SpinSign;
					Quaternion quaternion = Quaternion.CreateFromAxisAngle(Vector3.UnitY, angle);
					...
					if (IsBolt)
					{
						Vector3 vector2 = num2 * (float)((SpinSign == 1) ? 1 : (-1)) * Vector3.Up;
						ArtObject.Position = OriginalAoPosition + vector2;
						int num3 = 0;
						foreach (TrileInstance trile in AttachedGroup.Triles)
						{
							trile.Position = OriginalGroupTrilePositions[num3++] + vector2;
					...
					if (IsTimeswitch)
					{
						...
						{
							num4 = Math.Min(ScrewHeight, num2 / 2f) * 2f;
						}
						else if (SpinSign == 1 && ScrewHeight >= 1.5f)
						{
							num4 = Math.Min(2f - ScrewHeight, num2 / 2f) * 2f;
						}
						Vector3 vector3 = num4 * (float)((SpinSign == 1) ? 1 : (-1)) * Vector3.Up / 2f;
						TimeswitchScrewAo.Position = OriginalAoPosition + vector3;
						TimeswitchScrewAo.Rotation = OriginalScrewRotation * Quaternion.CreateFromAxisAngle(Vector3.UnitY, num4 * (MathF.PI / 2f) * (float)SpinSign * 2f);
					}
					if (SinceChanged.TotalSeconds >= 0.75)
					{
						...
						Vector3 vector4 = CameraManager.Viewpoint.ForwardVector();
						Vector3 vector5 = CameraManager.Viewpoint.DepthMask();
						Vector3 vector6 = (ArtObject.Position + CenterOffset) * vector5;
			...
			public void MoveToHeight()
			{
				...
				IWaiter waiter = Waiters.Interpolate(Math.Abs(movement.Y / 2f), delegate(float step)
				{
					float num = Easing.EaseInOut(step, EasingType.Sine);
					ArtObject.Position = Vector3.Lerp(origin, destination, num);
					ArtObject.Rotation = Quaternion.CreateFromAxisAngle(Vector3.UnitY, num * (MathF.PI / 2f) * (float)(int)Math.Round(movement.Y / 2f));
					foreach (TrileInstance trile in AttachedGroup.Triles)
					{
				...
				}, delegate
				{
					...
					ArtObject.Rotation = Quaternion.CreateFromAxisAngle(Vector3.UnitY, MathF.PI / 2f * (float)(int)Math.Round(movement.Y / 2f));
			...
			public void TrySpin()
			{
				...
				else
				{
					...
					Vector3 vector9 = (OriginalPlayerPosition = (PlayerManager.Position = PlayerManager.Position * vector4 + vector6 * vector7 - vector5 * 2f));
