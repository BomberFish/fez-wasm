	...
	internal class WatchersHost : DrawableGameComponent
	{
		...
		public override void Update(GameTime gameTime)
		{
			...
			foreach (TrileInstance key2 in watchers.Keys)
			{
				...
				float num = Vector3.Dot(vector8, vector);
				BoundingBox boundingBox = ((num > 0f) ? FezMath.Enclose(key2.Position + Vector3.UnitY * 0.05f - vector6, key2.Position + vector5 + vector6 + new Vector3(0.9f)) : FezMath.Enclose(key2.Position + Vector3.UnitY * 0.05f - vector6 - vector5, key2.Position + vector6 + new Vector3(0.9f)));
				...
				case WatcherAction.Idle:
				{
					...
					if (LevelManager.NearestTrile(key2.Position + new Vector3(0.5f)).Deep == key2 && (flag4 || flag5) && !FezMath.In(PlayerManager.Action, ActionType.GrabCornerLedge, ActionType.Suffering, ActionType.Dying, ActionTypeComparer.Default) && (watcherState3 = HasPair(key2)) != null)
					{
						...
						TimeSpan timeSpan2 = (watcherState3.StartTime = (watcherState2.StartTime = gameTime.TotalGameTime));
				...
				case WatcherAction.Crushing:
				{
					...
					if (!flag2)
					{
						...
						for (int i = 0; i < cornerCollision.Length; i++)
						{
							...
							if (pointCollision.Instances.Deep == key2)
							{
								flag2 = true;
								break;
							}
