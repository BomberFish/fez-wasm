	...
	internal class WaterfallsHost : GameComponent
	{
		private class WaterfallState
		{
			...
			public WaterfallState(BackgroundPlane plane, PlaneParticleSystem ps, WaterfallsHost host)
			{
				...
				{
					string text = (flag ? "sewer_small_splash" : "sewer_large_splash");
					animatedTexture = CMProvider.CurrentLevel.Load<AnimatedTexture>("Background Planes/sewer/" + text);
				}
				else if (LevelManager.WaterType == LiquidType.Purple)
				{
					string text2 = (flag ? "water_small_splash" : "water_large_splash");
					animatedTexture = CMProvider.CurrentLevel.Load<AnimatedTexture>("Background Planes/waterPink/" + text2);
				}
				else
				{
					string text3 = (flag ? "water_small_splash" : "water_large_splash");
					animatedTexture = CMProvider.CurrentLevel.Load<AnimatedTexture>("Background Planes/water/" + text3);
				}
				Splash = new BackgroundPlane(LevelMaterializer.AnimatedPlanesMesh, animatedTexture)
				{
					Doublesided = true,
					Crosshatch = true
				};
		...
		private void TryInitialize()
		{
			...
			foreach (BackgroundPlane backgroundPlane in array)
			{
				...
				else if (backgroundPlane.ActorType == ActorType.Drips)
				{
					...
					bool flag = backgroundPlane.Crosshatch || backgroundPlane.Billboard;
