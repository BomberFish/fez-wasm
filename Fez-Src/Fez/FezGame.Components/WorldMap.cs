...
namespace FezGame.Components
{
	public class WorldMap : DrawableGameComponent
	{
		...
		public override void Initialize()
		{
			...
			group = LegendMesh.AddFace(new Vector3(0.333f + 0.666f / num, 5.75f, 1f), new Vector3(0f, 0f, 0f), FaceOrientation.Front, centeredOnOrigin: false);
			group.Material = new Material
			{
				Diffuse = new Vector3(0.0625f)
			};
			group.Texture = CMProvider.Global.Load<Texture2D>("Other Textures/FullWhite");
			group = LegendMesh.AddFace(new Vector3(1f, 8f, 1f), new Vector3(0.25f, -2.5f, 0f), FaceOrientation.Front, centeredOnOrigin: false);
			group.Texture = CMProvider.Global.Load<Texture2D>("Other Textures/map_controls/icons_legend");
			WavesMesh = new Mesh
			{
				Effect = new DefaultEffect.Textured(),
				Blending = BlendingMode.Alphablending,
		...
		private void BuildNodes(MapNode node, MapNode.Connection parentConnection, MapNode parentNode, Vector3 offset, List<Matrix> instances)
		{
			...
			bool flag2 = (parentNode != null && GameState.SaveData.World.ContainsKey(GameState.IsTrialMode ? ("trial/" + parentNode.LevelName) : parentNode.LevelName)) || node.Connections.Any((MapNode.Connection connection) => GameState.SaveData.World.ContainsKey(GameState.IsTrialMode ? ("trial/" + connection.Node.LevelName) : connection.Node.LevelName));
			if (flag2 || flag || AllVisited || ShowAll)
			{
			...
			{
				if (c2.Node.NodeType == LevelNodeType.Lesser && node.Connections.Any((MapNode.Connection x) => x.Face == c2.Face && c2.Node.NodeType != LevelNodeType.Lesser))
				{
			...
			{
				c.MultiBranchId = node.Connections.Where((MapNode.Connection x) => x.Face == c.Face).Max((MapNode.Connection x) => x.MultiBranchId) + 1;
				c.MultiBranchCount = node.Connections.Count((MapNode.Connection x) => x.Face == c.Face);
			}
			float num = 0f;
			foreach (MapNode.Connection item in node.Connections.OrderByDescending((MapNode.Connection x) => x.Node.NodeType.GetSizeFactor()))
			{
				...
				bool flag3 = AllVisited || flag || GameState.SaveData.World.ContainsKey(GameState.IsTrialMode ? ("trial/" + item.Node.LevelName) : item.Node.LevelName);
				float num2 = 3f + (node.NodeType.GetSizeFactor() + item.Node.NodeType.GetSizeFactor()) / 2f;
				...
				{
					num2 -= (float)(item.Face.IsSide() ? 1 : 2);
				}
				num2 *= 1.25f + item.BranchOversize;
				...
				{
					vector2 = ((float)(item.MultiBranchId - 1) - (float)(item.MultiBranchCount - 1) / 2f) * (FezMath.XZMask - item.Face.AsVector().Abs()) * num3;
				}
				BuildNodes(item.Node, item, node, offset + vector * num2 + vector2, instances);
				if (flag3)
				{
					...
					if (item.MultiBranchCount > 1)
					{
						...
						instances.Add(new Matrix(vector4.X, vector4.Y, vector4.Z, 0f, 1f, 1f, 1f, 1f, vector3.X, vector3.Y, vector3.Z, 0f, 0f, 0f, 0f, 0f));
						float num4 = num2 - num;
						vector3 = vector * num4 + new Vector3(0.05375f);
						vector4 = vector * num4 / 2f + offset + vector * num + vector2;
						item.LinkInstances.Add(instances.Count);
						instances.Add(new Matrix(vector4.X, vector4.Y, vector4.Z, 0f, 1f, 1f, 1f, 1f, vector3.X, vector3.Y, vector3.Z, 0f, 0f, 0f, 0f, 0f));
					}
					else
					{
						Vector3 vector5 = vector * num2 + new Vector3(0.05375f);
						Vector3 vector6 = vector * num2 / 2f + offset;
						item.LinkInstances.Add(instances.Count);
						instances.Add(new Matrix(vector6.X, vector6.Y, vector6.Z, 0f, 1f, 1f, 1f, 1f, vector5.X, vector5.Y, vector5.Z, 0f, 0f, 0f, 0f, 0f));
					}
					DoSpecial(item, offset, vector, num2, instances);
				}
		...
		public override void Update(GameTime gameTime)
		{
			...
			CameraManager.Radius = ZoomCycle[ZoomLevel] * viewScale;
			...
			if (FinishedInTransition)
			{
				...
				flag2 = !FezMath.AlmostEqual(InputManager.Movement, Vector2.Zero);
				if (flag2)
				{
					CameraManager.Center += InputManager.Movement.X * right * (float)gameTime.ElapsedGameTime.TotalSeconds * (radius + 10f) * 0.4f / MathHelper.Lerp(viewScale, 1f, 0.5f) + InputManager.Movement.Y * up * (float)gameTime.ElapsedGameTime.TotalSeconds * (radius + 10f) * 0.4f / MathHelper.Lerp(viewScale, 1f, 0.5f);
				}
				if (flag)
				{
			...
			{
				double num = SinceStarted.TotalSeconds * 0.44999998807907104;
				for (int i = 0; i < WavesMesh.Groups.Count; i++)
				{
					float num2 = Easing.Ease(FezMath.Frac((float)num + (float)i / (float)(WavesMesh.Groups.Count * 2)), -0.5f, EasingType.Sine);
					float x = WavesMesh.Groups[i].Scale.X;
					WavesMesh.Groups[i].Scale = new Vector3(num2 * 5f * CurrentNode.NodeType.GetSizeFactor());
					if (i == 0 && x > WavesMesh.Groups[i].Scale.X)
					{
						sBeacon.EmitAt(WavesMesh.Position).OverrideMap = true;
					}
					WavesMesh.Groups[i].Material.Opacity = 1f - (num2 - 0.4f) / 0.6f;
				}
			}
			float num3 = (float)base.GraphicsDevice.Viewport.Width / (viewScale / 1280f);
			ScaleIcons();
			foreach (Group group2 in NodesMesh.Groups)
			{
				NodeGroupData nodeGroupData = group2.CustomData as NodeGroupData;
				...
				float num4 = nodeGroupData.Node.NodeType.GetSizeFactor() + 0.125f;
				...
				{
					vector = Vector3.Lerp(vector, new Vector3(1.25f * num4), 0.25f);
				}
				else if (!FezMath.AlmostEqual(vector.X, 1f))
				{
					vector = Vector3.Lerp(vector, new Vector3(num4), 0.25f);
				}
				Vector3 vector2 = group2.Position + nodeGroupData.Node.NodeType.GetSizeFactor() / 2f * new Vector3(1f, 1f, -1f) * (vector / num4) + 0.2f * new Vector3(1f, 0f, -1f);
				float iconScale = GetIconScale(viewScale, radius / viewScale / num3);
			...
			if (FocusNode != null && FocusNode.Group != null && !flag && !flag2)
			{
				...
				if (!FezMath.AlmostEqual(num5, 0f))
				{
					Vector3 vector6 = vector5 / num5;
					float num6 = MathHelper.Clamp(num5 * num5, 0f, (!chosenByMouseClick) ? 1 : 3);
					CameraManager.Center -= vector6 * num6 * radius * 0.005f;
		...
		private void StartInTransition()
		{
			...
			Waiters.Interpolate(0.75, delegate(float s)
			{
				if (base.Enabled)
				{
					...
					Quaternion quaternion5 = (iconsMesh3.Rotation = (linksMesh3.Rotation = quaternion2));
