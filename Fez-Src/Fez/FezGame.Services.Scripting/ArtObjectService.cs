	...
	public class ArtObjectService : IArtObjectService, IScriptingBase
	{
		...
		public void GlitchOut(int id, bool permanent, string spawnedActor)
		{
			if (LevelManager.ArtObjects.ContainsKey(id))
			{
				...
				{
					GlitchyDespawner glitchyDespawner = new GlitchyDespawner(ServiceHelper.Game, LevelManager.ArtObjects[id], LevelManager.ArtObjects[id].Position);
					glitchyDespawner.ActorToSpawn = (ActorType)Enum.Parse(typeof(ActorType), spawnedActor, ignoreCase: true);
					ServiceHelper.AddComponent(glitchyDespawner);
				}
				if (permanent)
				{
		...
		public LongRunningAction TiltOnVertex(int id, float durationSeconds)
		{
			return new LongRunningAction(delegate(float _, float totalSeconds)
			{
				...
				value.Rotation = Quaternion.CreateFromAxisAngle(Vector3.Right, (float)Math.Asin(Math.Sqrt(2.0) / Math.Sqrt(3.0)) * FezMath.Saturate(value2)) * Quaternion.CreateFromAxisAngle(Vector3.Up, MathF.PI / 4f * FezMath.Saturate(value2));
				return false;
			});
		}
		...
		public LongRunningAction Rotate(int id, float dX, float dY, float dZ)
		{
			return new LongRunningAction(delegate(float elapsedSeconds, float _)
			{
				...
				value.Rotation = Quaternion.CreateFromYawPitchRoll(dY * (MathF.PI * 2f) * elapsedSeconds, dX * (MathF.PI * 2f) * elapsedSeconds, dZ * (MathF.PI * 2f) * elapsedSeconds) * value.Rotation;
				return false;
			});
		}
		...
		public LongRunningAction HoverFloat(int id, float height, float cyclesPerSecond)
		{
			...
			return new LongRunningAction(delegate(float _, float sinceStarted)
			{
				...
				float num = (float)Math.Sin(sinceStarted * (MathF.PI * 2f) * cyclesPerSecond) * height;
		...
		{
			ArtObjectInstance artObjectInstance = LevelManager.ArtObjects[id];
			artObjectInstance.ActorSettings.ShouldMoveToEnd = true;
		}

		public void MoveNutToHeight(int id, float height)
		{
			ArtObjectInstance artObjectInstance = LevelManager.ArtObjects[id];
			artObjectInstance.ActorSettings.ShouldMoveToHeight = height;
		}
	}
}
