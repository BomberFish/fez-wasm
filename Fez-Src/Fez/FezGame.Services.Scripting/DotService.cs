...
namespace FezGame.Services.Scripting
{
	...
	{
		public static bool DisableDotScripting;
		...
		{
			if (DisableDotScripting)
			{
				if (hideAfter && LevelManager.Name == "VILLAGEVILLE_3D")
				{
					CameraManager.Constrained = false;
				}
				return new LongRunningAction((float _, float __) => true);
			}
			if (sTextNext == null)
			{
