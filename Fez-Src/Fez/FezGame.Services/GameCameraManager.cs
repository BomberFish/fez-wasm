...
namespace FezGame.Services
{
	public class GameCameraManager : DefaultCameraManager, IGameCameraManager, IDefaultCameraManager, ICameraProvider
	{
		...
		public override bool ActionRunning => Fez.LongScreenshot || base.ActionRunning || PlayerManager.Action == ActionType.PivotTombstone || PlayerManager.Action == ActionType.GrabTombstone;
		...
		public override float Radius
		{
			get
			{
				return GameState.MenuCubeIsZoomed ? 18f : base.Radius;
			}
			set
			{
				base.Radius = value;
		...
		protected override void PostUpdate()
		{
			if (!GameState.Loading)
			{
				...
				if (PlayerManager.CarriedInstance != null && shouldRotateInstance)
				{
					...
					float num2 = FezMath.WrapAngle(MathF.PI - (float)Math.Atan2(base.View.Forward.Z, base.View.Forward.X));
