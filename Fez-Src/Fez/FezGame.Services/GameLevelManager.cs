...
namespace FezGame.Services
{
	...
	{
		public static bool ResetCubeBitsOnLevelChange;
		...
		public void ChangeLevel(string levelName)
		{
			...
			PhysicsManager.Update(PlayerManager);
			PlayerManager.Velocity = 3.15f * (float)Math.Sign(CollisionManager.GravityFactor) * 0.15f * (1f / 60f) * -Vector3.UnitY;
			Vector3 vector4 = (CameraManager.InterpolatedCenter = (CameraManager.Center = PlayerManager.Center));
			OnLevelChanged();
			...
			{
				vector4 = (CameraManager.InterpolatedCenter = (CameraManager.Center = PlayerManager.Center + 4f * (float)((!base.Descending) ? 1 : (-1)) / CameraManager.PixelsPerTrixel * Vector3.UnitY));
			}
		...
		public void RemoveArtObject(ArtObjectInstance aoInstance)
		{
			if (aoInstance.ActorSettings.AttachedGroup.HasValue)
			{
				...
				TrileGroup trileGroup = base.Groups[aoInstance.ActorSettings.AttachedGroup.Value];
				TrileInstance[] array = trileGroup.Triles.ToArray();
