...
namespace FezGame.Services
{
	public class PhysicsManager : IPhysicsManager
	{
		...
		public WallHuggingResult HugWalls(IPhysicsEntity entity, bool determineBackground, bool postRotation, bool keepInFront)
		{
			...
			for (int i = 0; i < cornerCollision.Length; i++)
			{
				...
				if (trileInstance != null && (!(entity is ISimplePhysicsEntity) || trileInstance.PhysicsState == null || !trileInstance.PhysicsState.Puppet) && trileInstance.PhysicsState != entity)
				{
					...
					Vector3 vector4 = entity.Center - vector * entity.Size / 2f;
					Vector3 vector5 = vector4 - vector3 + num * -vector;
					float num2 = Vector3.Dot(vector5, vector);
					if (FezMath.AlmostClamp(num2) < 0f)
					{
						...
						{
							Vector3 vector6 = vector5 * vector.Abs();
							vector2 -= vector6;
							entity.Center -= vector6;
							result.Hugged = true;
						}
					}
				}
				...
				FaceOrientation o = CameraManager.LastViewpoint.VisibleOrientation();
				Vector3 vector7 = o.AsVector();
				...
				{
					Vector3 vector8 = trileInstance.Center + vector7 * trileInstance.TransformedSize.ZYX() / 2f;
					Vector3 vector9 = entity.Center - vector7 * entity.Size / 2f;
					Vector3 vector10 = vector9 - vector8 + 0.002f * vector7;
					float num4 = Vector3.Dot(vector10, vector7);
					if (FezMath.AlmostClamp(num4) < 0f && num4 > -1f && keepInFront)
					{
						Vector3 vector11 = vector10 * vector7.Abs();
						vector2 -= vector11;
						entity.Center -= vector11;
						result.Hugged = true;
					}
				}
			}
		...
		{
			return instance != null && instance.Enabled && !instance.Trile.Immaterial && (!instance.Trile.Thin || instance.Trile.ForceHugging) && instance != PlayerManager.CarriedInstance && instance != PlayerManager.PushedInstance && (!instance.Trile.ActorSettings.Type.IsBomb() || entity.Background) && (instance.PhysicsState == null || instance.PhysicsState != entity) && !FezMath.In(instance.GetRotatedFace(entity.Background ? CameraManager.VisibleOrientation.GetOpposite() : CameraManager.VisibleOrientation), CollisionType.Immaterial, CollisionType.TopNoStraightLedge, CollisionType.AllSides, CollisionTypeComparer.Default);
		}
		...
		public bool Update(ISimplePhysicsEntity entity, bool simple, bool keepInFront)
		{
			...
			if (!entity.IgnoreCollision)
			{
				...
				if ((flag ? (entity.Velocity.Y > 0f) : (entity.Velocity.Y < 0f)) && verticalResults.AnyCollided())
				{
					MultipleHits<TrileInstance> ground = entity.Ground;
					CollisionResult nearLow = verticalResults.NearLow;
					CollisionResult farHigh = verticalResults.FarHigh;
