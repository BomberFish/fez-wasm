...
namespace FezGame.Services
{
	public class PlayerManager : IPlayerManager, IComplexPhysicsEntity, IPhysicsEntity
	{
		...
		public bool Climbing => Action.IsClimbingLadder() || Action.IsClimbingVine();
		...
		public void RespawnAtCheckpoint()
		{
			...
			{
				Vector3 vector3 = (RespawnPosition = (Position = trileInstance.Center + (trileInstance.TransformedSize / 2f + Size / 2f) * Vector3.UnitY * Math.Sign(CollisionManager.GravityFactor)));
			}
			else
			{
				Vector3 vector3 = (RespawnPosition = (Position = GameState.SaveData.Ground + Size / 2f * Vector3.UnitY * Math.Sign(CollisionManager.GravityFactor)));
			}
