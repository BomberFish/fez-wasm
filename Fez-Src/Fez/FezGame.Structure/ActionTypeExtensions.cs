...
namespace FezGame.Structure
{
	public static class ActionTypeExtensions
	{
		...
		public static string GetAnimationPath(this ActionType action)
		{
			...
			default:
				throw new InvalidOperationException("Action '" + action.ToString() + "' does not map to an animation.");
			}
		}
		...
		{
			if (type == ActionType.Victory || type == ActionType.SuckedIn || (uint)(type - 104) <= 1u)
			{
				return true;
			}
			return false;
		...
		{
			if ((uint)(type - 53) <= 1u || type == ActionType.HurtSwim)
			{
				return true;
			}
			return false;
