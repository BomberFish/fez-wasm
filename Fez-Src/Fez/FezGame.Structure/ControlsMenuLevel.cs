...
namespace FezGame.Structure
{
	internal class ControlsMenuLevel : MenuLevel
	{
		...
		private bool mappedButton = false;
		...
		public override string AButtonString
		{
			get
			{
				...
				{
					return (base.SelectedItem.SuffixText != null) ? StaticText.GetString("ChangeWithGlyph") : StaticText.GetString("MenuApplyWithGlyph");
				}
				return null;
			}
		}
		...
		public ControlsMenuLevel(MenuBase menuBase)
		{
			...
			MenuItem menuItem = AddItem(null, MenuBase.SliderAction);
			menuItem.Selectable = false;
			AddItem("Vibration", delegate
			{
			}, defaultItem: false, () => SettingsManager.Settings.Vibration ? StaticText.GetString("On") : StaticText.GetString("Off"), delegate
			{
				ToggleVibration();
			});
			...
			menuItem = AddItem("ResetToDefault", delegate
			{
			...
			MenuItem<ArrowKeyMapping> menuItem2 = AddItem("ControlsMove", MenuBase.SliderAction, defaultItem: false, () => UpToAKM(kmap[MappedAction.Up]), delegate(ArrowKeyMapping lastValue, int change)
			{
				ArrowKeyMapping arrowKeyMapping2 = UpToAKM(kmap[MappedAction.Up]);
				...
				kmap[MappedAction.Up] = AKMToKey(arrowKeyMapping2, 0);
				kmap[MappedAction.Left] = AKMToKey(arrowKeyMapping2, 1);
				kmap[MappedAction.Down] = AKMToKey(arrowKeyMapping2, 2);
				kmap[MappedAction.Right] = AKMToKey(arrowKeyMapping2, 3);
				KeyboardManager.UpdateMapping();
				ValidateKeyCollision();
			});
			menuItem2.SuffixText = () => " : " + Localize(UpToAKM(kmap[MappedAction.Up]));
			MenuItem<ArrowKeyMapping> menuItem3 = AddItem("ControlsLook", MenuBase.SliderAction, defaultItem: false, () => UpToAKM(kmap[MappedAction.LookUp]), delegate(ArrowKeyMapping lastValue, int change)
			{
				ArrowKeyMapping arrowKeyMapping = UpToAKM(kmap[MappedAction.LookUp]);
				...
				kmap[MappedAction.LookUp] = AKMToKey(arrowKeyMapping, 0);
				kmap[MappedAction.LookLeft] = AKMToKey(arrowKeyMapping, 1);
				kmap[MappedAction.LookDown] = AKMToKey(arrowKeyMapping, 2);
				kmap[MappedAction.LookRight] = AKMToKey(arrowKeyMapping, 3);
				KeyboardManager.UpdateMapping();
				ValidateKeyCollision();
			});
			menuItem3.SuffixText = () => " : " + Localize(UpToAKM(kmap[MappedAction.LookUp]));
			...
			menuItem = AddItem(null, MenuBase.SliderAction);
			menuItem.Selectable = false;
			...
			menuItem = AddItem("Controller", MenuBase.SliderAction, defaultItem: true);
			menuItem.UpperCase = true;
			menuItem = AddItem("Keyboard", MenuBase.SliderAction, defaultItem: true);
			menuItem.UpperCase = true;
		}
			...
			if (base.TrapInput)
			{
				if (forGamepad)
				{
					...
					else if (thisButton != lastButton)
					{
						...
						if (num != 0)
						{
							int i = Items.IndexOf(keyGrabFor);
							MappedAction mappedAction2 = GamepadActionOrder[i];
							Dictionary<MappedAction, Buttons> controllerMapping = SettingsManager.Settings.ControllerMapping;
							Buttons value = controllerMapping[mappedAction2];
							if (thisButton.Start == ButtonState.Pressed)
							{
								controllerMapping[mappedAction2] = Buttons.Start;
							}
							else if (thisButton.Back == ButtonState.Pressed)
							{
								controllerMapping[mappedAction2] = Buttons.Back;
							}
							else if (thisButton.LeftStick == ButtonState.Pressed)
							{
								controllerMapping[mappedAction2] = Buttons.LeftStick;
							}
							else if (thisButton.RightStick == ButtonState.Pressed)
							{
								controllerMapping[mappedAction2] = Buttons.RightStick;
							}
							else if (thisButton.LeftShoulder == ButtonState.Pressed)
							{
								controllerMapping[mappedAction2] = Buttons.LeftShoulder;
							}
							else if (thisButton.RightShoulder == ButtonState.Pressed)
							{
								controllerMapping[mappedAction2] = Buttons.RightShoulder;
							}
							else if (thisButton.A == ButtonState.Pressed)
							{
								controllerMapping[mappedAction2] = Buttons.A;
							}
							else if (thisButton.B == ButtonState.Pressed)
							{
								controllerMapping[mappedAction2] = Buttons.B;
							}
							else if (thisButton.X == ButtonState.Pressed)
							{
								controllerMapping[mappedAction2] = Buttons.X;
							}
							else if (thisButton.Y == ButtonState.Pressed)
							{
								controllerMapping[mappedAction2] = Buttons.Y;
							}
							else if ((num & 0x400000) == 4194304)
							{
								controllerMapping[mappedAction2] = Buttons.RightTrigger;
							}
							else
							{
								...
								controllerMapping[mappedAction2] = Buttons.LeftTrigger;
							}
							if (keyGrabFor != gamepadFPItem || GameState.SaveData.HasFPView)
							{
								keyGrabFor.SuffixText = () => " : " + GamepadButtonOrder[i];
							}
							MappedAction? mappedAction3 = null;
							foreach (KeyValuePair<MappedAction, Buttons> item2 in controllerMapping)
							{
								if (item2.Value == controllerMapping[mappedAction2] && item2.Key != mappedAction2)
								{
									mappedAction3 = item2.Key;
									break;
								}
							}
							if (mappedAction3.HasValue)
							{
								controllerMapping[mappedAction3.Value] = value;
								int j;
								for (j = 0; j < GamepadActionOrder.Length; j++)
								{
									if (GamepadActionOrder[j] != mappedAction3.Value)
									{
										continue;
									}
									if (GamepadActionOrder[j] != MappedAction.FpViewToggle || GameState.SaveData.HasFPView)
									{
										Items[j].SuffixText = () => " : " + GamepadButtonOrder[j];
									}
									break;
								}
							}
				...
				else
				{
					...
					foreach (Keys item3 in keysDown)
					{
						...
						MappedAction mappedAction = KeyboardActionOrder[num2 - keyboardStart];
						Dictionary<MappedAction, Keys> kMap = SettingsManager.Settings.KeyboardMapping;
						kMap[mappedAction] = value2;
						if (keyGrabFor != keyboardFPItem || GameState.SaveData.HasFPView)
						{
							keyGrabFor.SuffixText = () => " : " + Localize(kMap[mappedAction]);
						}
						KeyboardManager.UpdateMapping();
						ValidateKeyCollision();
						base.TrapInput = false;
