	...
	internal class MenuItem<T> : MenuItem
	{
		...
		public string Text
		{
			get
			{
				...
				return UpperCase ? text.ToUpper(CultureInfo.InvariantCulture) : text;
			}
			set
			{
				text = value;
		...
		public float ActivityRatio
		{
			get
			{
				float num = SinceHovered.Ticks;
				TimeSpan hoverGrowDuration = HoverGrowDuration;
				return FezMath.Saturate(num / (float)hoverGrowDuration.Ticks);
			}
		}
