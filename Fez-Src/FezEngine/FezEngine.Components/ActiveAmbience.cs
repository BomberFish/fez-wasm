...
namespace FezEngine.Components
{
	internal class ActiveAmbience : GameComponent
	{
		...
		public void ChangeTracks(IEnumerable<AmbienceTrack> tracks)
		{
			foreach (ActiveAmbienceTrack activeTrack in ActiveTracks)
			{
				foreach (AmbienceTrack track in tracks)
				{
					if (track.Name == activeTrack.Track.Name)
					{
						...
						bool flag = activeTrack.ActiveForDayPhase && !activeTrack.ForceMuted && !activeTrack.WasMuted;
						activeTrack.WasMuted = false;
						bool flag2 = TimeManager.IsDayPhaseForMusic(DayPhase.Day) && track.Day;
						flag2 |= TimeManager.IsDayPhaseForMusic(DayPhase.Night) && track.Night;
						flag2 |= TimeManager.IsDayPhaseForMusic(DayPhase.Dawn) && track.Dawn;
						flag2 = (activeTrack.ActiveForDayPhase = flag2 | (TimeManager.IsDayPhaseForMusic(DayPhase.Dusk) && track.Dusk));
						if (flag != activeTrack.ActiveForDayPhase && !activeTrack.ForceMuted)
						{
			...
			{
				bool flag4 = TimeManager.IsDayPhaseForMusic(DayPhase.Day) && item2.Day;
				flag4 |= TimeManager.IsDayPhaseForMusic(DayPhase.Night) && item2.Night;
				flag4 |= TimeManager.IsDayPhaseForMusic(DayPhase.Dawn) && item2.Dawn;
				flag4 |= TimeManager.IsDayPhaseForMusic(DayPhase.Dusk) && item2.Dusk;
				ActiveTracks.Add(new ActiveAmbienceTrack(item2, flag4));
			}
		}
