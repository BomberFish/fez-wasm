...
namespace FezEngine.Components
{
	public class FarawayPlaceHost : DrawableGameComponent
	{
		private class PlaceFader : DrawableGameComponent
		{
			...
			public override void Draw(GameTime gameTime)
			{
				...
				if (GameState.StereoMode && rth != null)
				{
					...
					Texture2D texture2D2 = (CombineEffect.RightTexture = (CombineEffect.LeftTexture = rth.Target));
		...
		private void TryInitialize()
		{
			...
			{
				ScriptTrigger scriptTrigger = item.Triggers.FirstOrDefault((ScriptTrigger x) => x.Object.Type == "Volume" && x.Event == "Enter");
				int? identifier = scriptTrigger.Object.Identifier;
				...
				DrawActionScheduler.Schedule(delegate
				{
					string text2 = "Other Textures/faraway_thumbs/" + destLevelName + " (" + destFace.AsViewpoint().ToString() + ")";
					...
					if (!ThisLevelMesh.Groups.Any((Group x) => x.Texture == texture))
					{
						...
						object obj4 = (group2.CustomData = (group.CustomData = farawayPlaceData2));
						Vector3 vector3 = (group2.Position = (group.Position = farawayPlaceData2.OriginalCenter));
						Texture texture4 = (group2.Texture = (group.Texture = texture));
		...
		public override void Update(GameTime gameTime)
		{
			...
			if (EngineState.FarawaySettings.InTransition && EngineState.FarawaySettings.OriginFadeOutStep == 1f && !IsFake)
			{
				...
				ThisLevelMesh.ClearGroups();
				float num2 = (OriginalFakeRadius = (float)base.GraphicsDevice.Viewport.Width / (CameraManager.PixelsPerTrixel * 16f));
		...
		{
			double num = 0.0;
			num += 12.0 * Math.Pow(pixelsPerTrixel, -1.0);
			return num + -2.0;
		}
		...
		private void PositionFarawayPlaces()
		{
			...
			for (int i = 0; i < ThisLevelMesh.Groups.Count; i++)
			{
				...
				float num2 = CameraManager.PixelsPerTrixel;
				if (EngineState.FarawaySettings.InTransition && FezMath.AlmostEqual(EngineState.FarawaySettings.DestinationCrossfadeStep, 1f))
				{
					num2 = MathHelper.Lerp(CameraManager.PixelsPerTrixel, EngineState.FarawaySettings.DestinationPixelsPerTrixel, (EngineState.FarawaySettings.TransitionStep - 0.875f) / 0.125f);
				}
				float num3 = (float)(-4 * ((!LevelManager.Descending) ? 1 : (-1))) / num2 - 15f / 32f + 1f;
				Vector3 vector2 = CameraManager.InterpolatedCenter - farawayPlaceData.OriginalCenter + num3 * Vector3.UnitY;
				float num4 = (float)GetCustomOffset(num2) * (float)((!LevelManager.Descending) ? 1 : (-1)) + 15f / 32f;
				float num5 = 0f;
				if (flag && farawayPlaceData.WaterLevelOffset.HasValue)
				{
					vector2 *= FezMath.XZMask;
					vector *= Vector2.UnitX;
					num5 = farawayPlaceData.WaterLevelOffset.Value - num4 / 4f - 0.5f + 0.125f;
					farawayPlaceData.Volume.ActorSettings.WaterOffset = num5;
				}
				Vector3 vector5 = (group.Position = (group2.Position = (farawayPlaceData.OriginalCenter + (farawayPlaceData.DestinationOffset + num4 * Vector3.UnitY) / 4f) * farawayPlaceData.Viewpoint.ScreenSpaceMask() + farawayPlaceData.Viewpoint.DepthMask() * CameraManager.InterpolatedCenter + farawayPlaceData.Viewpoint.ForwardVector() * 30f * num + farawayPlaceData.Viewpoint.RightVector() * vector.X + Vector3.Up * vector.Y + vector2 * farawayPlaceData.Viewpoint.ScreenSpaceMask() / 2f + num5 * Vector3.UnitY));
