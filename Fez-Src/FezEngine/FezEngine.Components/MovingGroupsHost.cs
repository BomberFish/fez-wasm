	...
	public class MovingGroupsHost : DrawableGameComponent
	{
		...
		private void TrackGroupConnectivePath(int groupId)
		{
			...
			if (value.MoveToEnd)
			{
				...
				Vector3 vector4 = artObjectInstance.Position - zero;
				value.Triles.Sort(new MovingTrileInstanceComparer(vector4));
				foreach (TrileInstance trile2 in value.Triles)
				{
					trile2.Position += vector4;
			...
			{
				Vector3 vector5 = trile3.TransformedSize / 2f;
				boundingBox.Min = Vector3.Min(boundingBox.Min, (trile3.Center - vector5) * vector);
				boundingBox.Max = Vector3.Max(boundingBox.Max, (trile3.Center + vector5) * vector + vector3);
				zero2 += trile3.Center;
			}
			...
			foreach (ArtObjectInstance connectedAo4 in connectedAos)
			{
				...
				{
					Vector3 vector5 = connectedAo4.Scale * connectedAo4.ArtObject.Size / 2f;
					boundingBox2 = new BoundingBox(connectedAo4.Position - vector5, connectedAo4.Position + vector5);
					Quaternion quaternion = connectedAo4.Rotation;
					FezMath.RotateOnCenter(ref boundingBox2, ref quaternion);
					boundingBox2.Min *= vector;
					boundingBox2.Max = boundingBox2.Max * vector + vector3;
			...
			foreach (ArtObjectInstance connectedAo5 in connectedAos)
			{
				...
				{
					Vector3 vector5 = connectedAo5.Scale * connectedAo5.ArtObject.Size / 2f;
					BoundingBox box = new BoundingBox((connectedAo5.Position - vector5) * vector, (connectedAo5.Position + vector5) * vector + vector3);
			...
			while (true)
			{
				...
				Vector3 vector6 = artObjectInstance3.ActorSettings.PrecedingNodeAo.Position - artObjectInstance3.Position;
				if (!(Math.Abs(vector6.Dot(vector3)) > Math.Abs(vector6.Dot(FezMath.XZMask - vector3))))
				{
					bool flag = false;
					Vector3 point = vector6 / 2f + artObjectInstance3.Position;
					foreach (ArtObjectInstance connectedAo6 in connectedAos)
					{
						if (connectedAo6.ArtObject.ActorType == ActorType.None)
						{
							Vector3 vector5 = connectedAo6.Scale * connectedAo6.ArtObject.Size / 2f;
							boundingBox2 = new BoundingBox(connectedAo6.Position - vector5, connectedAo6.Position + vector5);
			...
			Vector3 vector7 = artObjectInstance2.Position - zero2;
			value.Triles.Sort(new MovingTrileInstanceComparer(vector7));
			foreach (TrileInstance trile4 in value.Triles)
			{
				trile4.Position += vector7;
			...
			while (true)
			{
				...
				Vector3 vector8 = nextNodeAo.Position - artObjectInstance4.Position;
				bool flag2 = Math.Abs(vector8.Dot(vector3)) > Math.Abs(vector8.Dot(FezMath.XZMask - vector3));
				bool flag3 = false;
				Vector3 point2 = vector8 / 2f + artObjectInstance4.Position;
				foreach (ArtObjectInstance connectedAo7 in connectedAos)
				{
					if (connectedAo7.ArtObject.ActorType == ActorType.None)
					{
						Vector3 vector5 = connectedAo7.Scale * connectedAo7.ArtObject.Size / 2f;
						BoundingBox boundingBox3 = new BoundingBox(connectedAo7.Position - vector5, connectedAo7.Position + vector5);
		...
		private void InterpolationCallback(GameTime gameTime)
		{
			...
			double totalSeconds = (gameTime.TotalGameTime - TimeInterpolation.LastUpdate).TotalSeconds;
			TimeSpan updateTimestep = TimeInterpolation.UpdateTimestep;
			double num = totalSeconds / updateTimestep.TotalSeconds;
			foreach (MovingGroupState trackedGroup in trackedGroups)
			{
