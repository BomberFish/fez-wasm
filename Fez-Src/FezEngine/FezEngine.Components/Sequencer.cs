	...
	public abstract class Sequencer : GameComponent
	{
		...
		protected void LoadCrystalSamples()
		{
			...
			foreach (TrileInstance item in crystals.Keys.Where((TrileInstance x) => x.ActorSettings.SequenceSampleName != null))
			{
				...
				{
					Logger.Log("Sequencer", LogSeverity.Warning, "Could not find crystal sample : " + crystalState.Sample?.ToString() + " or " + crystalState.AlternateSample);
				}
		...
		public override void Update(GameTime gameTime)
		{
			...
			long ticks = SoundManager.PlayPosition.Ticks;
			double num = FezMath.Frac((double)ticks / (double)measureLength.Ticks);
			...
			foreach (TrileInstance key in crystals.Keys)
			{
				...
				else
				{
					...
					double num4 = (num2 - 0.44999998807907104) / 0.550000011920929;
					int num5 = (int)Math.Round(num4 * 6.0);
					if (num5 % 3 == 0)
					{
						if (!key.Hidden)
						{
