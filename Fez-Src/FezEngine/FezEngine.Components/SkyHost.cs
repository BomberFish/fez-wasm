	...
	public class SkyHost : DrawableGameComponent
	{
		...
		private void InitializeSky()
		{
			...
			if (LevelManager.Sky.CloudTint != null)
			{
				DrawActionScheduler.Schedule(delegate
				{
					Texture2D texture2D3 = cm.Load<Texture2D>(skyPath + LevelManager.Sky.CloudTint);
					cloudColors = new Color[texture2D3.Width];
					texture2D3.GetData(0, new Rectangle(0, texture2D3.Height / 2, texture2D3.Width, 1), cloudColors, 0, texture2D3.Width);
				});
			}
			else
			{
			...
			DrawActionScheduler.Schedule(delegate
			{
				...
				{
					Texture2D texture2D = cm.Load<Texture2D>(skyPath + layer3.Name);
					...
					foreach (FaceOrientation value2 in Util.GetValues<FaceOrientation>())
					{
						if (value2.IsSide())
						{
							BgLayers.Groups[num11++].Texture = ((texture2D2 != null && value2 != 0) ? texture2D2 : texture2D);
						}
					}
				}
			});
			...
			DrawActionScheduler.Schedule(delegate
			{
				...
				{
					cloudMeshes[num10].Effect = new CloudsEffect();
					cloudMeshes[num10].Texture = cm.Load<Texture2D>(skyPath + cloud3);
					num10++;
				}
			});
			float num3 = 64f * LevelManager.Sky.Density;
			int num4 = (int)Math.Sqrt(num3);
			float num5 = num3 / (float)num4;
			float num6 = RandomHelper.Between(0.0, 6.2831854820251465);
			...
			if (cloudMeshes.Count > 0)
			{
				for (int i = 0; i < num4; i++)
				{
					...
					{
						Mesh mesh = RandomHelper.InList(cloudMeshes);
						Group group2 = mesh.AddFace(Vector3.One, Vector3.Zero, FaceOrientation.Front, centeredOnOrigin: true);
						float num8 = RandomHelper.Between(0.0, 1f / (float)num4 * (MathF.PI * 2f));
						float num9 = RandomHelper.Between(0.0, 1f / num5 * 192f);
						Layer key = RandomHelper.EnumField<Layer>();
						cloudStates[key].Add(new CloudState
						{
							Group = group2,
							Phi = ((float)i / (float)num4 * (MathF.PI * 2f) + num6 + num8) % (MathF.PI * 2f),
							LocalHeightOffset = ((float)j / num5 * 96f * 2f + num7 + num9) % 192f - 96f
						});
		...
		private void ForceUpdate(GameTime gameTime)
		{
			...
			Vector3 vector = ((LevelManager.Size == Vector3.Zero) ? new Vector3(16f) : LevelManager.Size);
			...
			float num2 = Math.Abs(vector.Dot(CameraManager.Viewpoint.RightVector())) / 32f;
			...
			if (CameraManager.Viewpoint.IsOrthographic() && LevelManager.Sky.HorizontalScrolling)
			{
				...
				{
					float num3 = num * LevelManager.Sky.WindSpeed * 0.025f;
					BgLayerState bgLayerState = group.CustomData as BgLayerState;
					bgLayerState.WindOffset += num3;
				}
			}
			ShootStars();
			ResizeStars();
		...
		private void MoveAndRotateClouds(GameTime gameTime)
		{
			...
			foreach (Layer key in cloudStates.Keys)
			{
				...
				Vector3 vector3 = ((!flag) ? (vector2 + Vector3.One * (32f + 48f * key.DistanceFactor())) : (vector2 + Vector3.One * 32f / 2.5f * num3));
				foreach (CloudState item in cloudStates[key])
				{
					if (flag)
					{
					...
					Vector3 vector4 = new Vector3(y: item.GetHeight((!flag) ? 1f : (CameraManager.ProjectionTransition ? MathHelper.Lerp(1f, 0.2f, CameraManager.ViewTransitionStep) : 0.2f)), x: (float)Math.Sin(item.Phi) * vector3.X + vector2.X, z: (float)Math.Cos(item.Phi) * vector3.Z + vector2.Z);
					Quaternion quaternion = Quaternion.CreateFromYawPitchRoll(item.Phi, MathF.PI, MathF.PI);
					if (CameraManager.ProjectionTransition)
					{
						if (flag)
						{
							...
							item.ActualVisibility = (vector4 - vector2).Dot(forward) <= 0f;
						}
						else
						{
					...
					{
						bool flag2 = (vector4 - vector2).Dot(forward) <= 0f;
		...
		public void ForceDraw(GameTime gameTime)
		{
			...
			if (RtHandle != null)
			{
				...
				float num = ((EngineState.FarawaySettings.InterpolatedFakeRadius == 0f) ? RadiusAtFirstDraw : EngineState.FarawaySettings.InterpolatedFakeRadius);
				float num2 = num / RadiusAtFirstDraw;
				Matrix matrix = new Matrix(1f, 0f, 0f, 0f, 0f, 1f, 0f, 0f, -0.5f, -0.5f, 1f, 0f, 0f, 0f, 0f, 1f);
				Matrix matrix2 = new Matrix(num2, 0f, 0f, 0f, 0f, num2, 0f, 0f, 0f, 0f, 1f, 0f, 0f, 0f, 0f, 1f);
				Matrix matrix3 = new Matrix(1f, 0f, 0f, 0f, 0f, 1f, 0f, 0f, 0.5f, 0.5f, 1f, 0f, 0f, 0f, 0f, 1f);
				float num3 = num2 * (1f - EngineState.FarawaySettings.DestinationCrossfadeStep);
				TargetRenderer.DrawFullscreen(RtHandle.Target, matrix * matrix2 * matrix3, new Color(num3, num3, num3));
			}
			if (!flag2 || flickering)
			{
			...
			if ((LevelManager.Rainy && !flag2) || LevelManager.Sky.Name == "WATERFRONT")
			{
				...
				for (int i = 0; i < 3; i++)
				{
					for (int j = i * num4; j < count && j < (i + 1) * num4; j++)
					{
						BgLayers.Groups[j].Enabled = !BgLayers.Groups[j].AlwaysOnTop.GetValueOrDefault();
					}
					graphicsDevice.PrepareStencilWrite((StencilMask)(5 + i));
					BgLayers.Draw();
					for (int k = i * num4; k < count && k < (i + 1) * num4; k++)
					{
			...
			else
			{
				...
				foreach (Group group in BgLayers.Groups)
				{
					group.Enabled = !group.AlwaysOnTop.GetValueOrDefault();
				}
				BgLayers.Draw();
