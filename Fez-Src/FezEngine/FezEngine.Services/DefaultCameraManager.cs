...
namespace FezEngine.Services
{
	public class DefaultCameraManager : CameraManager, IDefaultCameraManager, ICameraProvider
	{
		...
		public virtual float Radius
		{
			get
			{
				return ProjectionTransition ? current.Radius : interpolated.Radius;
			}
			set
			{
				bool flag = !FezMath.AlmostEqual(current.Radius, value);
				current.Radius = value;
				if (flag && viewpoint.IsOrthographic())
				{
					RebuildProjection();
				}
			}
		...
		public Viewpoint LastViewpoint => (lastViewpoint == Viewpoint.Perspective) ? olderViewpoint : lastViewpoint;

		public bool ProjectionTransitionNewlyReached => transitionNewlyReached && lastViewpoint == Viewpoint.Perspective;

		public bool DollyZoomOut { protected get; set; }

		public bool ViewTransitionReached => (directionTransition == null || directionTransition.Reached) && !ForceTransition;

		public virtual bool ActionRunning => ViewTransitionReached;

		public float ViewTransitionStep => (directionTransition == null || ViewTransitionReached) ? 0f : directionTransition.TotalStep;
		...
		public void AlterTransition(Viewpoint newTo)
		{
			...
			Viewpoint fromView2 = FezMath.OrientationFromDirection(directionTransition.Points[2]).AsViewpoint();
			int distance = fromView2.GetDistance(newTo);
		...
		{
			Vector3 vector = ((!FezMath.AlmostEqual(FezMath.AngleBetween(from, to), MathF.PI)) ? FezMath.Slerp(from, to, 0.5f) : Vector3.Cross(Vector3.Normalize(to - from), Vector3.UnitY));
		...
		{
			float num = (viewpoint.IsOrthographic() ? ((FarPlane - NearPlane) / 2f) : current.Radius);
			Vector3 cameraPosition = num * current.Direction + current.Center;
		...
		{
			float num = (viewpoint.IsOrthographic() ? 249.875f : interpolated.Radius);
			Vector3 cameraPosition = num * interpolated.Direction + interpolated.Center;
