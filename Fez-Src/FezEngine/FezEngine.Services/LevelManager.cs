...
namespace FezEngine.Services
{
	public abstract class LevelManager : GameComponent, ILevelManager
	{
		...
		public event Action ScreenInvalidated = null;
		...
		public Trile SafeGetTrile(int trileId)
		{
			...
			return (TrileSet == null) ? fallbackTrile : TrileSet[trileId];
		}

		public TrileInstance TrileInstanceAt(ref TrileEmplacement id)
		{
			TrileInstance value;
			return Triles.TryGetValue(id, out value) ? value : null;
		}
		...
		public bool IsBorderTrileFace(ref TrileEmplacement id, ref FaceOrientation face)
		{
			...
			TrileInstance value;
			return !levelData.Triles.TryGetValue(traversal, out value) || value.Trile.SeeThrough || value.ForceSeeThrough;
		}
		...
		public bool IsInRange(ref TrileEmplacement id)
		{
			...
			return id.X >= 0 && (float)id.X < size.X && id.Y >= 0 && (float)id.Y < size.Y && id.Z >= 0 && (float)id.Z < size.Z;
		}

		public bool IsInRange(Vector3 position)
		{
			Vector3 size = levelData.Size;
			return position.X >= 0f && position.X < size.X && position.Y >= 0f && position.Y < size.Y && position.Z >= 0f && position.Z < size.Z;
		}
		...
		public TrileInstance ActualInstanceAt(Vector3 position)
		{
			...
			return OffsetInstanceAt(emplacement, screenSpacePosition, flag, forwardSign, useSelector: false, keepNearest: false, QueryOptions.None, out queryResult) ?? ((num < 0.5f) ? OffsetInstanceAt(emplacement.GetOffset((!flag) ? (-1) : 0, 0, flag ? (-1) : 0), screenSpacePosition, flag, forwardSign, useSelector: false, keepNearest: false, QueryOptions.None, out queryResult) : OffsetInstanceAt(emplacement.GetOffset((!flag) ? 1 : 0, 0, flag ? 1 : 0), screenSpacePosition, flag, forwardSign, useSelector: false, keepNearest: false, QueryOptions.None, out queryResult));
		}
		...
		private static bool OffsetInstanceContains(Vector3 screenSpacePosition, TrileInstance instance, bool depthIsZ)
		{
			...
			return screenSpacePosition.X > vector.X - vector2.X && screenSpacePosition.X < vector.X + vector2.X && screenSpacePosition.Y >= vector.Y - vector2.Y && screenSpacePosition.Y < vector.Y + vector2.Y && (screenSpacePosition.Z == -1f || (screenSpacePosition.Z > vector.Z - vector2.Z && screenSpacePosition.Z < vector.Z + vector2.Z));
		}
		...
		private static TrileInstance KeepNearestInstance(TrileInstance nearest, TrileInstance contender, bool depthIsZ, int forwardSign)
		{
			...
			return (num > num2) ? nearest : contender;
		}

		public IEnumerable<Trile> ActorTriles(ActorType type)
		{
			return (TrileSet == null) ? Enumerable.Repeat<Trile>(null, 1) : TrileSet.Triles.Values.Where((Trile x) => x.ActorSettings.Type == type);
		}
