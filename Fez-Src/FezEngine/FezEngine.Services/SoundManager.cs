...
namespace FezEngine.Services
{
	public class SoundManager : GameComponent, ISoundManager
	{
		...
		public TimeSpan PlayPosition => (ActiveSong == null) ? TimeSpan.Zero : ActiveSong.PlayPosition;

		public TrackedSong CurrentlyPlayingSong => (ActiveSong == null) ? null : ActiveSong.Song;
		...
		public void InitializeLibrary()
		{
			...
			for (int i = 0; i < num; i++)
			{
				...
				{
					MusicCache.Add(text, binaryReader.ReadBytes(num2));
				}
			}
		}
		...
		public void FadeFrequencies(bool toLowPass, float fadeDuration)
		{
			if (!IsLowPass && toLowPass)
			{
				...
				freqTransitionWaiter = (thisWaiter2 = Waiters.Interpolate(fadeDuration * (1f - originalStep2), delegate(float s)
				{
					if (freqTransitionWaiter == thisWaiter2)
					{
						FrequencyStep = originalStep2 + s * (1f - originalStep2);
			...
			else if (IsLowPass && !toLowPass)
			{
				...
				freqTransitionWaiter = (thisWaiter = Waiters.Interpolate(fadeDuration * originalStep, delegate(float s)
				{
					if (freqTransitionWaiter == thisWaiter)
					{
						FrequencyStep = originalStep * (1f - s);
		...
		public void KillSounds()
		{
			lock (Emitters)
			{
				...
				SoundEmitter[] array2 = array;
				foreach (SoundEmitter soundEmitter in array2)
				{
		...
		{
			string path = Path.Combine(Path.Combine(CMProvider.Global.RootDirectory, "Sounds"), "SoundLevels.sdl");
			string cleanPath = SharedContentManager.GetCleanPath(path);
