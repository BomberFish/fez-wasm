		...
		private void UpdateBuffers(bool rebuild)
		{
			...
			if (!useHwInstancing)
			{
				DrawActionScheduler.Schedule(delegate
				{
					...
					{
						((DynamicVertexBuffer)vertexBuffer).SetData(tempVertices, 0, tempVertices.Length, SetDataOptions.Discard);
						((DynamicIndexBuffer)indexBuffer).SetData(tempIndices, 0, tempIndices.Length, SetDataOptions.Discard);
					}
			...
			if (useHwInstancing)
			{
				...
				if (InstancesDirty)
				{
					if (appendIndex)
					{
						...
						instanceBuffer.SetData(indexedInstances, 0, num, SetDataOptions.Discard);
					}
					else
					{
						instanceBuffer.SetData(Instances, 0, num, SetDataOptions.Discard);
					}
