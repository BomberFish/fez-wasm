...
namespace FezEngine.Structure
{
	public static class ActorTypeExtensions
	{
		...
		{
			if (type == ActorType.Bomb || (uint)(type - 16) <= 2u)
			{
				return true;
			}
			return false;
		...
		{
			if (type == ActorType.None || (uint)(type - 46) <= 2u || type == ActorType.BigWaterfall)
			{
				return true;
			}
			return false;
		...
		{
			return type == ActorType.None || (!type.SupportsArtObjects() && !type.SupportsGroups() && (!type.SupportsNPCs() & !type.SupportsPlanes()));
		}
		...
		{
			if (type == ActorType.PushSwitch || (uint)(type - 29) <= 1u)
			{
				return true;
			}
			return false;
		...
		{
			return type.IsPickable() && !type.IsLight();
		}
