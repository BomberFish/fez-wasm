...
namespace FezEngine.Structure
{
	public class BackgroundPlane
	{
		...
		private void InitializeGroup()
		{
			...
			{
				Group groupCopy = (Group = HostMesh.AddFace(Size, Vector3.Zero, FaceOrientation.Front, centeredOnOrigin: true, doublesided, crosshatch));
		...
		public BackgroundPlane Clone()
		{
			BackgroundPlane backgroundPlane = new BackgroundPlane
			{
				...
				TextureName = TextureName,
				Texture = Texture,
				Timing = ((Timing == null) ? null : Timing.Clone()),
				Position = position,
				Rotation = rotation,
				Scale = scale,
				LightMap = lightMap,
				AllowOverbrightness = allowOverbrightness,
				Filter = filter,
				Doublesided = doublesided,
				Opacity = opacity,
				Crosshatch = crosshatch,
				SyncWithSamples = SyncWithSamples,
				AlwaysOnTop = alwaysOnTop,
				Fullbright = fullbright,
				Loop = Loop,
				Billboard = billboard,
				AttachedGroup = AttachedGroup,
				YTextureRepeat = YTextureRepeat,
				XTextureRepeat = XTextureRepeat,
				ClampTexture = ClampTexture,
