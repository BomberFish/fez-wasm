...
namespace FezEngine.Structure
{
	public class OggStream : IDisposable
	{
		...
		public bool IsStopped => soundEffect.State == SoundState.Stopped || soundEffect.PendingBufferCount == 0;
		...
		private static IntPtr TellCallback(IntPtr datasource)
		{
			if (!Streams.TryGetValue(datasource.ToInt32(), out var value))
			{
				return IntPtr.Zero;
			}
			return (IntPtr)value.streamOffset;
		}
		...
		public static void SyncAllFilter(MethodInfo applyFilter, object[] gainContainer)
		{
			foreach (OggStream value in Streams.Values)
			{
				if (value.LowPass && value.soundEffect != null)
				{
