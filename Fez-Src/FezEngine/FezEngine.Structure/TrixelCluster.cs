...
namespace FezEngine.Structure
{
	public class TrixelCluster : ISpatialStructure<TrixelEmplacement>, IDeserializationCallback
	{
		public class Chunk
		{
			...
			internal bool Empty => Boxes.Count == 0 && Trixels.Count == 0;
			...
			internal void ConsolidateTrixels()
			{
				...
				while (stack.Count > 0)
				{
					...
					while (hashSet.Count > 1)
					{
						...
						HashSet<TrixelEmplacement> hashSet2 = VisitChunk(origin, hashSet);
						stack.Push(hashSet2);
		...
		public class Box
		{
			...
			internal bool Contains(TrixelEmplacement trixel)
			{
				return trixel.X >= Start.X && trixel.Y >= Start.Y && trixel.Z >= Start.Z && trixel.X < End.X && trixel.Y < End.Y && trixel.Z < End.Z;
			}
