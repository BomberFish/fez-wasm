...
namespace FezEngine.Structure
{
	public class WinConditions
	{
		...
		{
			return UnlockedDoorCount >= wonditions.UnlockedDoorCount && LockedDoorCount >= wonditions.LockedDoorCount && ChestCount >= wonditions.ChestCount && CubeShardCount >= wonditions.CubeShardCount && OtherCollectibleCount >= wonditions.OtherCollectibleCount && SplitUpCount >= wonditions.SplitUpCount && SecretCount >= wonditions.SecretCount && wonditions.ScriptIds.All((int x) => ScriptIds.Contains(x));
		}
