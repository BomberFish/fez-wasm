...
namespace FezEngine.Tools
{
	public class ArtObjectMaterializer
	{
		...
		{
			return missingTrixels.Empty || !missingTrixels.IsFilled(trixelIdentifier);
		}

		private bool CanContain(TrixelEmplacement trixel)
		{
			return (float)trixel.X < size.X * 16f && (float)trixel.Y < size.Y * 16f && (float)trixel.Z < size.Z * 16f && trixel.X >= 0 && trixel.Y >= 0 && trixel.Z >= 0;
		}

		private bool IsBorderTrixelFace(TrixelEmplacement traversed)
		{
			return !CanContain(traversed) || !TrixelExists(traversed);
		}
		...
		public void UpdateSurfaces()
		{
			...
			foreach (TrixelFace tf2 in array2)
			{
				...
				foreach (TrixelSurface item in surfaces.Where((TrixelSurface x) => x.Orientation == tf2.Face && x.Trixels.Contains(tf2.Id)))
				{
					item.Trixels.Remove(tf2.Id);
			...
			array = added.ToArray();
			Array.Sort(array, invalidTrixelFaceComparer);
			TrixelFace[] array3 = array;
			foreach (TrixelFace tf in array3)
			{
				TrixelSurface[] array4 = surfaces.Where((TrixelSurface x) => x.CanContain(tf.Id, tf.Face)).ToArray();
				if (array4.Length != 0)
				{
					TrixelSurface trixelSurface2 = array4[0];
					trixelSurface2.Trixels.Add(tf.Id);
				...
				{
					surfaces.Add(new TrixelSurface(tf.Face, tf.Id));
				}
			}
			added.Clear();
			RebuildParts();
		...
		public void RebuildGeometry()
		{
			...
			{
				for (ushort num3 = 0; num3 < 6; num3 = (ushort)(num3 + 1))
				{
