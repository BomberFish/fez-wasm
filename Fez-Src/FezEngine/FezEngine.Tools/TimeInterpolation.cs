...
namespace FezEngine.Tools
{
	public static class TimeInterpolation
	{
		...
		private static readonly List<OrderedCallback> thisUpdateInterpolationCallbacks = new List<OrderedCallback>();
		...
		public static void ProcessCallbacks(GameTime gameTime)
		{
			lock (interpolationCallbacks)
			{
				thisUpdateInterpolationCallbacks.AddRange(interpolationCallbacks);
			}
			foreach (OrderedCallback thisUpdateInterpolationCallback in thisUpdateInterpolationCallbacks)
			{
				thisUpdateInterpolationCallback.Callback(gameTime);
			}
			thisUpdateInterpolationCallbacks.Clear();
		}
	}
}
